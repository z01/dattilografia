...
testo[a++]='GConf (GNOME configuration library)\n\
GConf is a configuration database system, functionally similar to the\n\
Windows registry.\n\
Homepage: http://www.gnome.org/projects/gconf/';

testo[a++]='M2Crypto (cryptography toolkit for Python)\n\
M2Crypto is a crypto and SSL toolkit for Python. It includes:\n\
- RSA, DSA, DH, HMACs, message digests, symmetric ciphers (e.g. AES)\n\
- SSL functionality to implement clients and servers\n\
- HTTPS extensions to Python\'s httplib, urllib, and xmlrpclib\n\
- Unforgeable HMAC\'ing AuthCookies for web session management\n\
- FTP/TLS client and server, S/MIME v2, ZServerSSL, ZSmime\n\
Website: http://wiki.osafoundation.org/bin/view/Projects/MeTooCrypto';

testo[a++]='MPlayer (media player)\n\
...
testo[a++]='ORBit2 (CORBA ORB library)\n\
...
testo[a++]='PyQt (Python bindings for Qt)\n\
PyQt is a set of Python bindings for Trolltech\'s Qt application\n\
framework and runs on all platforms supported by Qt.\n\
Homepage: http://www.riverbankcomputing.co.uk/software/PyQt/';

testo[a++]='QScintilla (Qt port of the Scintilla C++ editor control)\n\
...
testo[a++]='a2ps (any to PostScript filter)\n\
...
testo[a++]='aaa_base (Basic Linux filesystem package)\n\
...
testo[a++]='aaa_elflibs (shared libraries needed by many programs)\n\
...
testo[a++]='aaa_terminfo (a basic collection of terminfo entries)\n\
...
testo[a++]='aalib (ASCII Art library) _1l1vlvlvlvlvlvlvlvlvvl=.\n\
...
testo[a++]='acct (process accounting utilities)\n\
...
Manuel A. Fernandez Montecelo, and Tim Schmielau.';

testo[a++]='acl (tools for using POSIX Access Control Lists)\n\
...
testo[a++]='akonadi (PIM storage service)\n\
...
testo[a++]='alpine (Alternatively Licensed Program for Internet News and Email)\n\
...
Setup command.\n\
The Alpine site may be found here: http://www.washington.edu/alpine';

testo[a++]='alsa-lib (Advanced Linux Sound Architecture library)\n\
...
testo[a++]='alsa-oss (library/wrapper to use OSS programs with ALSA)\n\
The Advanced Linux Sound Architecture (ALSA) provides audio and MIDI\n\
...
testo[a++]='alsa-utils (Advanced Linux Sound Architecture utilities)\n\
The Advanced Linux Sound Architecture (ALSA) provides audio and MIDI\n\
...
testo[a++]='amarok (music player)\n\
Amarok is a music player for Linux and Unix with an intuitive\n\
interface. Amarok makes playing the music you love easier than ever\n\
before -- and looks good doing it.\n\
Amarok\'s home page is: http://amarok.kde.org';

testo[a++]='amp (An MP3 player)\n\
amp (Audio Mpeg Player) is a command-line MPEG audio decoder (MP3\n\
player). It works with both MPEG1 and MPEG2 audio streams (except for\n\
the multichannel extensions defined in MPEG2), layers 2 and 3.';

testo[a++]='anthy (a system for Japanese input method)\n\
...
testo[a++]='apmd (Advanced Power Management daemon)\n\
...
testo[a++]='apr (Apache Portable Runtime)\n\
The mission of the Apache Portable Runtime (APR) is to provide a\n\
free library of C data structures and routines, forming a system\n\
portability layer to as many operating systems as possible.';

testo[a++]='apr-util (Apache Portable Runtime utilities)\n\
The mission of the Apache Portable Runtime (APR) is to provide a\n\
free library of C data structures and routines, forming a system\n\
portability layer to as many operating systems as possible.\n\
...
testo[a++]='ash (Kenneth Almquist\'s ash shell)\n\
...
testo[a++]='at (delayed command execution)\n\
...
testo[a++]='atk (accessibility functions library)\n\
atk is a library of accessibility functions that is used by the\n\
GTK+-2 toolkit.';

testo[a++]='attica (Qt Collaboration library)\n\
Attica is a Qt library that implements the freedesktop.org\n\
specification for the Open Collaboration Services API version 1.4.\n\
It grants easy access to the services such as querying information\n\
about persons and contents.\n\
Homepage: http://www.kde.org';

testo[a++]='attr (tools for using extended attributes on filesystems)\n\
...
testo[a++]='audacious (A relatively powerful media player)\n\
...
testo[a++]='audacious-plugins (Plugins used by the Audacious media player)\n\
This is a collection of plugins for Audacious to support additional\n\
media formats.\n\
The webpage for Audacious is: http://www.audacious-media-player.org.';

testo[a++]='audiofile (audio support library)\n\
...
testo[a++]='aumix (GTK+/ncurses audio mixer)\n\
...
testo[a++]='autoconf (generate configuration scripts)\n\
...
testo[a++]='autofs (kernel-based automounter)\n\
...
testo[a++]='automake (a Makefile generator)\n\
...
testo[a++]='automoc4 (automatic moc for Qt4)\n\
This package contains the automoc4 binary which is used to run moc\n\
on the right source files in a Qt 4 or KDE 4 application.\n\
Moc is the meta object compiler which is a widely used tool with Qt\n\
and creates standard C++ files to provide syntactic sugar of the\n\
signal/slots mechanism.\n\
See also http://www.kde.org';

testo[a++]='babl (pixel format translation library)\n\
...
testo[a++]='bash (sh-compatible shell)\n\
...
testo[a++]='bash-completion (programmable completion for the bash shell)\n\
...
Homepage: https://alioth.debian.org/projects/bash-completion/';

testo[a++]='bc (An arbitrary precision calculator language)\n\
...
testo[a++]='biff+comsat (mail arrival notification daemon)\n\
...
testo[a++]='binutils (GNU binary development tools)\n\
...
testo[a++]='bison (parser generator similar to yacc)\n\
...
testo[a++]='blackbox (Blackbox window manager)\n\
...
testo[a++]='Blueman (Graphical Bluetooth Manager)\n\
...
testo[a++]='bluez (Bluetooth libraries and utilities)\n\
Bluez is the Bluetooth stack for Linux, allowing Bluetooth adaptors\n\
and devices to be used with Linux. This package contains the Bluez\n\
libraries, utilities, and other support files.\n\
For more info, visit: http://www.bluez.org';

testo[a++]='bluez-firmware (Bluetooth(TM) firmware)\n\
The bluez-firmware package contains firmware needed by various\n\
chipsets to support Bluetooth(TM) on Linux.\n\
For more info, visit: http://www.bluez.org';

testo[a++]='bluez-hcidump (Bluetooth analyzer)\n\
The bluez-hcidump package contains a network analyzer for Bluetooth.\n\
This utility (hcidump) utility reads raw HCI data coming from and\n\
going to a Bluetooth device and prints commands, events and data.\n\
For more info, visit: http://www.bluez.org';

testo[a++]='boost (Boost C++ Libraries)\n\
...
testo[a++]='bpe (Binary Patch Editor)\n\
...
testo[a++]='bridge-utils (network Ethernet bridging utilities)\n\
...
testo[a++]='bsd-finger (user information lookup program)\n\
...
testo[a++]='bsd-games (Classic BSD text games collection)\n\
...
testo[a++]='btrfs-progs (Btrfs filesystem utilities)\n\
Btrfs is a new copy on write filesystem for Linux aimed at implementing\n\
advanced features while focusing on fault tolerance, repair and easy\n\
administration. Initially developed by Oracle, Btrfs is licensed under\n\
the GPL and open for contribution from anyone. Btrfs is under heavy\n\
development, and is not suitable for any uses other than benchmarking\n\
and review. The Btrfs disk format is not yet finalized.\n\
Btrfs homepage: http://btrfs.wiki.kernel.org';

testo[a++]='bzip2 (a block-sorting file compressor)\n\
...
testo[a++]='ca-certificates (PEM Files of CA Certificates)\n\
This package includes PEM files of CA certificates to allow SSL-based\n\
applications to check for the authenticity of SSL connections.\n\
Homepage: http://packages.qa.debian.org/c/ca-certificates.html';

testo[a++]='cairo (graphics library used by GTK+)\n\
...
testo[a++]='cdrdao (CD burning utility)\n\
...
testo[a++]='cdrtools (CD-R, ISO and related tools)\n\
...
testo[a++]='chmlib (library for accessing CHM files)\n\
chmlib is a small library designed for accessing MS ITSS files.\n\
The ITSS file format is used for Microsoft Html Help files (.chm),\n\
which have been the predominant medium for software documentation\n\
from Microsoft during the past several years, having superceded\n\
the previously used .hlp file format.\n\
Homepage: http://www.jedrea.com/chmlib/';

testo[a++]='clisp (a Common Lisp implementation)\n\
...
testo[a++]='compositeproto\n\
compositeproto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='coreutils (core GNU utilities)\n\
...
testo[a++]='cpio (backup and archiving utility)\n\
...
testo[a++]='cpufrequtils (Kernel CPUfreq utilities)\n\
The cpufrequtils package makes access to the Linux kernel CPUfreq\n\
subsystem easier for users and cpufreq userspace tools. It contains\n\
a library used by other programs (libcpufreq), and command line tools\n\
to determine current CPUfreq settings and to modify them.\n\
Dominik Brodowski is the cpufrequtils maintainer.\n\
The cpufrequtils page is hosted on kernel.org:\n\
http://www.kernel.org/pub/linux/utils/kernel/cpufreq/cpufreq.html';

testo[a++]='crda (Central Regulatory Domain Agent)\n\
This is the Central Regulatory Domain Agent for Linux.\n\
It serves one purpose: tell the Linux kernel what to enforce.\n\
In essence, it is a udev helper for communication between\n\
the kernel and userspace.\n\
You only need to run this manually for debugging purposes.\n\
http://wireless.kernel.org/download/crda/';

testo[a++]='cryptsetup (utility for setting up encrypted filesystems)\n\
...
testo[a++]='cscope (source code browsing tool)\n\
...
testo[a++]='CUPS (Common UNIX Printing System)\n\
...
testo[a++]='curl (command line URL data transfer tool)\n\
...
testo[a++]='cvs (Concurrent Versions System)\n\
...
testo[a++]='cxxlibs (C++ shared library compatibility package)\n\
...
testo[a++]='cyrus-sasl (Simple Authentication and Security Layer)\n\
...
testo[a++]='db42 (Berkeley database library version 4.2.x)\n\
...
databases created with the Berkeley DB version 4.2.x.';

testo[a++]='db4 (Berkeley database library version 4.4.x)\n\
The Berkeley Database (Berkeley DB) library provides embedded database\n\
support for both traditional and client/server applications.\n\
This package should be installed if compatibility is needed with\n\
databases created with the Berkeley DB version 4.4.x.';

testo[a++]='dbus (D-Bus message bus system)\n\
...
testo[a++]='dbus-glib (Glib bindings for dbus)\n\
This package includes the glib bindings for the dbus IPC library.';

testo[a++]='dbus-python (Python bindings for dbus)\n\
This package includes python bindings for the D-Bus messagebus.';

testo[a++]='dc3dd (version of dd designed for computer forensics)\n\
dc3dd is a patched version of GNU dd to include a number of features\n\
useful for computer forensics. Many of these features were inspired by\n\
dcfldd, but were rewritten for dc3dd. Added features include robust\n\
error handling, logging, progress meter, image verification, split\n\
output and more.\n\
http://dc3dd.sourceforge.net/';

testo[a++]='dcron (Dillon\'s Cron daemon)\n\
...
testo[a++]='ddrescue (automatic data recovery tool)\n\
GNU ddrescue is a data recovery tool. It copies data from one file or\n\
block device (hard disc, cdrom, etc) to another, trying hard to rescue\n\
data in case of read errors. Every time you run it on the same output\n\
file, it tries to fill in the gaps.\n\
ddrescue home: http://www.gnu.org/software/ddrescue/ddrescue.html';

testo[a++]='dejavu-fonts-ttf (DejaVu fonts)\n\
...
testo[a++]='desktop-file-utils (Utilities for manipulating desktop files)\n\
Some utilities from freedesktop.org for handling .desktop files:\n\
...
desktop-menu-tool: loads .vfolder-info and .menu files and does\n\
operations on them.';

testo[a++]='dev86 (8086 development utilities)\n\
...
testo[a++]='devs (system device files)\n\
...
testo[a++]='dhcp (DHCP server and client utilities)\n\
...
testo[a++]='dhcpcd (DHCP client daemon)\n\
...
testo[a++]='dialog (display dialog boxes from shell scripts)\n\
...
testo[a++]='diffstat (produces statistics from diff output)\n\
...
gpgsm (from gnupg 2.x) or when running as a system daemon through\n\
the dirmngr-client tool.';

testo[a++]='distcc (distributed C/C++ compiler/daemon)\n\
...
testo[a++]='djvulibre (web-centric document and image format)\n\
...
testo[a++]='dmapi (Data Management API library)\n\
A library required to use the Data Management API (DMAPI).\n\
This interface is defined in the X/Open document \'Systems Management:\n\
Data Storage Managment (XDSM) API\' dated February 1997.\n\
The DMAPI library is used by the xfsdump utility.';

testo[a++]='dmidecode (DMI table decoder)\n\
...
testo[a++]='dnsmasq (small DNS and DHCP server)\n\
...
testo[a++]='dosfstools (tools for working with FAT filesystems)\n\
Utilities for creating FAT filesystems (mkdosfs), and for checking\n\
and repairing them (dosfsck).';

testo[a++]='doxygen (documentation generator)\n\
...
testo[a++]='dri2proto\n\
dri2proto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='dvd+rw-tools (DVD+-RW tools)\n\
...
testo[a++]='e2fsprogs (ext2 and ext3 filesystems utilities)\n\
...
testo[a++]='ebook-tools (access and convert various ebook file formats)\n\
Tools for accessing and converting various ebook file formats.\n\
Home page: http://sourceforge.net/projects/ebook-tools/';

testo[a++]='ed (text editor)\n\
...
testo[a++]='Eigen2 (c++ math library)\n\
Eigen 2 is a lightweight C++ template library for vector and matrix\n\
math, a.k.a. linear algebra.\n\
See the website: http://eigen.tuxfamily.org';

testo[a++]='eject (a tool to eject removable media)\n\
...
testo[a++]='electricsheep (distributed screensaver)\n\
Electric Sheep is an xscreensaver module that displays mpeg video\n\
of an animated fractal flame. In the background it contributes\n\
...
testo[a++]='elm (Menu-driven user mail program)\n\
...
testo[a++]='Enchant (a wrapper for spellcheck libraries)\n\
Enchant supports:\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Aspell/Pspell\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Ispell\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js MySpell/HunSpell\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Uspell (Yiddish, Hebrew and Eastern European languages)\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Hspell (Hebrew) and others\n\
...
testo[a++]='enscript (converts text files to PostScript and other formats)\n\
Enscript converts text files to PostScript and spools the generated\n\
output to a printer or saves it to a file. If no input files are\n\
given, enscript processes standard input. Enscript can be extended to\n\
handle different output media and it has many options for customizing\n\
printouts. ';

testo[a++]='epic5 (Internet Relay Chat client)\n\
...
testo[a++]='esound (Enlightened Sound Daemon)\n\
...
testo[a++]='fetchmail (mail retrieval and forwarding utility)\n\
...
testo[a++]='fftw (Fastest Fourier Transform in the West)\n\
...
testo[a++]='file (a utility to determine file type)\n\
...
testo[a++]='findutils (utilities to locate files)\n\
...
testo[a++]='flac (Free Lossless Audio Codec)\n\
...
testo[a++]='flex (fast lexical analyzer generator)\n\
...
testo[a++]='floppy (floppy disk utilities)\n\
...
testo[a++]='font-arabic-misc\n\
font-arabic-misc is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='font-bh-100dpi\n\
font-bh-100dpi is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='font-bh-75dpi\n\
font-bh-75dpi is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='font-bh-lucidatypewriter-100dpi\n\
font-bh-lucidatypewriter-100dpi is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='font-bh-lucidatypewriter-75dpi\n\
...
testo[a++]='font-winitzki-cyrillic\n\
font-winitzki-cyrillic is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='font-xfree86-type1\n\
font-xfree86-type1 is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='fontcacheproto\n\
fontcacheproto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='fontconfig (Font library and tools)\n\
...
testo[a++]='foomatic-filters (Universal print filter/RIP wrapper)\n\
...
For more information on foomatic, see: http://linuxprinting.org';

testo[a++]='freetype (A Free, High-Quality, and Portable Font Engine)\n\
The FreeType engine is a free and portable TrueType font rendering\n\
engine. It has been developed to provide TrueType support to a\n\
great variety of platforms and environments.';

testo[a++]='fribidi (Unicode BiDirectional algorithm library)\n\
...
testo[a++]='FUSE (Filesystem in Userspace)\n\
...
The FUSE web site can be found here: http://fuse.sourceforge.net';

testo[a++]='fvwm (a small and fast virtual window manager for X)\n\
...
testo[a++]='gcc (Base GCC package with C support)\n\
...
needed to compile Fortran code. The gcc package is also required.';

testo[a++]='gcc-gnat (Ada support for GCC)\n\
...
testo[a++]='gcc-java (Java support for GCC)\n\
...
testo[a++]='gcc-objc (Objective-C support for GCC)\n\
...
testo[a++]='gdb (the GNU symbolic debugger)\n\
...
testo[a++]='gdbm (GNU database routines)\n\
GNU dbm is a set of database routines that work similar to the\n\
standard UNIX dbm routines.';

testo[a++]='gdisk (GPT fdisk)\n\
GPT fdisk (gdisk) is a partitioning tool for disks using a GPT (GUID\n\
Partition Table). It features a command-line interface, fairly direct\n\
manipulation of partition table structures, recovery tools to help\n\
you deal with corrupt partition tables, and the ability to convert\n\
MBR disks to GPT format.\n\
gdisk home: http://www.rodsbooks.com/gdisk/';

testo[a++]='gdk-pixbuf2 (image library used by GTK+ v2)\n\
...
testo[a++]='geeqie (Geeqie image viewer)\n\
...
testo[a++]='GEGL (Generic Graphics Library)\n\
...
testo[a++]='genpower (UPS monitoring daemon)\n\
The genpower daemon, genpowerd, will monitor the status of a serial\n\
line connected to a UPS (Uninterruptible Power Supply). If a power\n\
failure is detected, genpowerd will notify the system to take the\n\
needed steps to react to the condition of the UPS. This may include\n\
shutting the system down, or canceling a pending shutdown if the\n\
power is restored.';

testo[a++]='getmail (POP3 mail retriever)\n\
...
testo[a++]='gettext (internationalization framework)\n\
...
testo[a++]='gettext-tools (internationalization framework tools)\n\
...
testo[a++]='getty-ps (console or terminal login)\n\
...
testo[a++]='gftp (FTP client for X)\n\
gFTP is a multithreaded FTP client for X written using GTK+. It\n\
...
file transfers, drag-n-drop support, bookmarks menu, stop button,\n\
and many more features.';

testo[a++]='ghostscript (GPL Ghostscript)\n\
...
testo[a++]='ghostscript-fonts-std (fonts for the Ghostscript interpreter)\n\
These fonts are used by the GhostScript interpreter.';

testo[a++]='giflib (GIF image format library)\n\
...
testo[a++]='gimp (The GNU Image Manipulation Program)\n\
...
testo[a++]='git (the stupid content tracker)\n\
...
testo[a++]='gkrellm (GNU Krell Monitors)\n\
...
testo[a++]='glew (the OpenGL Extension Wrangler Library)\n\
...
testo[a++]='glib (library of C routines, version 1.x)\n\
...
testo[a++]='glib2 (library of C routines)\n\
GLib is a library which includes support routines for C such as lists,\n\
trees, hashes, memory allocation, and many other things.';

testo[a++]='glibc (GNU C libraries)\n\
...
testo[a++]='glibc-i18n (locale files from glibc)\n\
...
testo[a++]='glibc-profile (GNU C libraries with profiling support)\n\
...
testo[a++]='glibc-solibs (shared GNU C libraries)\n\
...
testo[a++]='glibc-zoneinfo (timezone database)\n\
...
sub-matrices, dense and sparse solvers, etc.) for any interfaced vector\n\
type or matrix type. It can be viewed as a glue library that allows\n\
cooperation between several vector and matrix types. However, basic\n\
sparse, dense and skyline matrix/vector types are built in Gmm++, so\n\
it can be used as a standalone linear algebra library.';

testo[a++]='gmp (GNU multiple precision arithmetic library)\n\
...
testo[a++]='gnome-icon-theme (default icons used by GTK+)\n\
A set of default icons used by many GTK+ applications.';

testo[a++]='gnuchess (GPL licensed chess engines and graphical frontends)\n\
This is a package of programs for playing chess. It includes the\n\
gnuchess and Sjeng chess engines, and the eboard and XBoard\n\
graphical interfaces to these engines.';

testo[a++]='gnupg (The GNU Privacy Guard)\n\
GnuPG is GNU\'s tool for secure communication and data storage. It can\n\
be used to encrypt data and to create digital signatures. It includes\n\
an advanced key management facility and is compliant with the proposed\n\
OpenPGP Internet standard as described in RFC2440.';

testo[a++]='GnuPG2 (The GNU Privacy Guard version 2.x)\n\
GnuPG is GNU\'s tool for secure communication and data storage. It can\n\
be used to encrypt data and to create digital signatures. It includes\n\
an advanced key management facility and is compliant with the proposed\n\
...
testo[a++]='gnuplot (plotting utility)\n\
...
testo[a++]='gnutls (GNU TLS library)\n\
...
testo[a++]='gpa (GNU Privacy Assistant)\n\
...
testo[a++]='gperf (a perfect hash function generator)\n\
...
testo[a++]='gphoto2 (A command-line frontend to libgphoto2)\n\
libgphoto2 is a cross-platform digital camera library, and gphoto2 is\n\
a command-line client for it.\n\
Visit the gphoto project online: http://www.gphoto.org';

testo[a++]='gpm (general purpose mouse server)\n\
...
testo[a++]='grep (print lines matching a pattern)\n\
...
testo[a++]='groff (document formatting system)\n\
...
form in the /usr/man/man1 /usr/man/man2 /usr/man/man3 /usr/man/man4 /usr/man/man5 /usr/man/man6 /usr/man/man7 /usr/man/man8 /usr/man/man9 /usr/man/mann directories.';

testo[a++]='gst-plugins-base (base set of GStreamer plugins)\n\
...
testo[a++]='gst-plugins-good (good set of GStreamer plugins)\n\
GStreamer Good Plug-ins is a set of plug-ins that have good\n\
quality code, correct functionality, and a good license\n\
(LGPL for the plug-in code, LGPL or LGPL-compatible for the\n\
supporting library).';

testo[a++]='GStreamer (streaming multimedia framework)\n\
...
testo[a++]='gtk+ (graphical user interface toolkit, version 1.x)\n\
...
testo[a++]='GTK+ version 2 (multi-platform GUI toolkit)\n\
...
testo[a++]='gtkspell (GTK2 spell checker interface library)\n\
...
testo[a++]='gucharmap (character map utility)\n\
gucharmap is a Unicode character map and font viewer.';

testo[a++]='guile (GNU\'s extension language library)\n\
...
testo[a++]='gutenprint (IJS printer driver for Ghostscript and CUPS)\n\
...
testo[a++]='gxine (GTK+ media player based on Xine)\n\
...
testo[a++]='gzip (file compression utility)\n\
Gzip reduces the size of the named files using Lempel-Ziv coding\n\
(LZ77). Whenever possible, each file is replaced by one with the\n\
extension .gz, while keeping the same ownership modes, access and\n\
modification times.';

testo[a++]='hal (The HAL Hardware Abstraction Layer)\n\
...
the insertion of a DVD, or a USB flash memory stick).\n\
For more info, see: http://hal.freedesktop.org/';

testo[a++]='hal-info (device information for HAL)\n\
The hal-info package contains device information for HAL, a Hardware\n\
Abstraction Layer.\n\
For more info, see: http://www.freedesktop.org/Software/hal';

testo[a++]='hdparm (read/set hard drive parameters)\n\
...
testo[a++]='hfsutils (HFS volume utilities)\n\
...
testo[a++]='hicolor-icon-theme (default icon theme)\n\
...
testo[a++]='hplip (HP print/scan/fax support)\n\
...
testo[a++]='htdig (web indexing and searching system)\n\
The ht://Dig system is a complete web indexing and searching system\n\
for a small domain or intranet. This system is not indended to\n\
replace the need for outside Internet search engines such as Google,\n\
but rather to cover the search needs for a single company, campus, or\n\
even a particular sub section of a web site.';

testo[a++]='htop (ncurses-based interactive process viewer)\n\
...
testo[a++]='httpd (The Apache HTTP Server)\n\
...
testo[a++]='hunspell (spell checker and morphological analyzer library)\n\
...
library, Ispell pipe interface, OpenOffice.org UNO module.\n\
...
testo[a++]='icmpinfo (ICMP monitoring utility)\n\
...
testo[a++]='icon-naming-utils (generate icon files for your DE)\n\
It is a script for maintaining backwards compatibility\n\
with current desktop icon themes, while migrating to\n\
the names specified in the Icon Naming Specification.\n\
Homepage: http://tango.freedesktop.org';

testo[a++]='idnkit (Internationalized domain names toolkit)\n\
idnkit is a toolkit for handling internationalized domain names.\n\
It includes a library for handling internationalized domain names\n\
(libidnkit, libidnkitlite), a codeset conversion utility (idnconv),\n\
and a command which adds IDN dynamically to applications (runidn).\n\
Homepage: http://www.nic.ad.jp/en/idn/';

testo[a++]='ilmbase (Industrial Light & Magic base libraries)\n\
ilmbase currently includes:\n\
IlmThread - a thread abstraction library\n\
Imath - a math function library\n\
Iex - an excpetion-handling library\n\
Half - Ilm\'s 16 bit floating point format library\n\
Homepage: http://www.openexr.org/';

testo[a++]='imagemagick (a robust collection of image processing tools)\n\
...
testo[a++]='imapd (IMAP4rev1 from alpine)\n\
...
testo[a++]='imlib (image loading and rendering library)\n\
...
testo[a++]='indent (changes the appearance of a C program)\n\
...
testo[a++]='inetd (Internet super-server)\n\
...
testo[a++]='infozip (Info-ZIP\'s zip and unzip utilities)\n\
...
testo[a++]='inotify-tools (command line utilities for inotify)\n\
...
testo[a++]='intel-gpu-tools (Intel DRM Driver Testing Tools)\n\
This is a collection of tools for development and testing of the Intel\n\
DRM driver. There are many macro-level test suites including xtest,\n\
rendercheck, piglit, and oglconform.';

testo[a++]='intltool (Utilities for translation support)\n\
This package contains scripts and autoconf files for\n\
internationalizing data.';

testo[a++]='iproute2 (IP routing utilities)\n\
...
testo[a++]='iptables (IP packet filter administration)\n\
...
header, and much more. See: http://www.netfilter.org';

testo[a++]='iptraf-ng (console-based network monitoring program)\n\
...
iptraf-ng was written by Gerard Paul Java.';

testo[a++]='iputils (a collection of common network tools)\n\
...
testo[a++]='ipw2100-fw (Intel ipw2100 wireless firmware)\n\
...
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='ipw2200-fw (Intel ipw2200 wireless firmware)\n\
...
Your use of this firmware is governed by this Intel license:\n\
/usr/doc/ipw2200-fw-3.1/LICENSE\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='irssi (Internet Relay Chat client)\n\
...
For more information, visit: http://irssi.org\n\
irssi was written by Timo Sirainen.';

testo[a++]='iso-codes (ISO-standard lists)\n\
This package provides lists of various ISO standards (e.g.\n\
country, language, language scripts, and currency names) in one\n\
place, rather than repeated in many programs throughout the system.\n\
Homepage: http://pkg-isocodes.alioth.debian.org/';

testo[a++]='ispell (spelling checker)\n\
...
testo[a++]='iw (tool for configuring Linux wireless devices)\n\
...
testo[a++]='iwlwifi-1000-ucode (Intel Centrino N 100 wireless microcode)\n\
The microcode provided in this package is required for the Intel\n\
Wireless WiFi Link AGN driver for Linux (iwlagn) to operate.\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='iwlwifi-1000-ucode (Intel 1000 wireless microcode)\n\
The microcode provided in this package is required for the Intel\n\
Wireless WiFi Link AGN driver for Linux (iwlagn) to operate.\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='iwlwifi-3945-ucode (Intel 3945 wireless microcode)\n\
The microcode provided in this package is required for the Intel\n\
PRO/Wireless 3945ABG/BG Network Connection Adapter driver for Linux\n\
(iwlwifi-3945) to operate.\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='iwlwifi-4965-ucode (Intel 4965 wireless microcode)\n\
The microcode provided in this package is required for the Intel\n\
PRO/Wireless 4965ABG/BG Network Connection Adapter driver for Linux\n\
(iwlwifi-4965) to operate.\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='iwlwifi-5000-ucode (Intel 5000 wireless microcode)\n\
The microcode provided in this package is required for the Intel\n\
PRO/Wireless 5000ABG/BG Network Connection Adapter driver for Linux\n\
(iwlagn) to operate.\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='iwlwifi-5150-ucode (Intel 5150 wireless microcode)\n\
The microcode provided in this package is required for the Intel\n\
PRO/Wireless 5150AGN Network Connection Adapter driver for Linux\n\
(iwlagn) to operate.\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='iwlwifi-6000-ucode (Intel 6000 wireless microcode)\n\
The microcode provided in this package is required for the Intel\n\
Wireless WiFi Link AGN driver for Linux (iwlagn) to operate.\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='iwlwifi-6xxx-ucode (Intel 6xxx wireless microcode)\n\
The microcode provided in this package is required for the Intel\n\
Wireless WiFi Link AGN driver for Linux (iwlagn) to operate.\n\
Visit Intel\'s Linux wireless website at this address:\n\
http://intellinuxwireless.org/';

testo[a++]='jasper (free implementation of the JPEG-2000 standard)\n\
...
testo[a++]='Java(TM) 2 Platform Standard Edition Development Kit.\n\
...
testo[a++]='jed (programmer\'s editor)\n\
JED (by John E. Davis) is an extremely powerful but small emacs-like\n\
programmer\'s editor that is extensible in a C-like macro language and\n\
can do color syntax highlighting. Among the many features:\n\
Emacs, wordstar, EDT emulation;\n\
C, Fortran, TeX, text editing modes; Full undo;\n\
GNU Emacs compatible info reader, and lots more.\n\
JED is also 8 bit clean, so you can even edit binary files! ';

testo[a++]='jfsutils (IBM JFS utilities)\n\
...
testo[a++]='joe (Joe text editor)\n\
Joseph H. Allen\'s easy to use text editor, similar to WordStar[tm].';

testo[a++]='jove (Jonathan\'s Own Version of Emacs)\n\
...
testo[a++]='kaudiocreator (CD ripper and audio encoder)\n\
kaudiocreater is a KDE frontend for ripping and encoding audio\n\
to/from optical media.';

testo[a++]='kbd (keyboard maps and console fonts)\n\
...
testo[a++]='kdeaccessibility (Accessibility programs for KDE)\n\
...
testo[a++]='kdeadmin (System Administration tools)\n\
KDE system administration tools.';

testo[a++]='kdeartwork (Artwork for KDE)\n\
Additional themes, screensavers, sounds, wallpapers, and window\n\
styles.';

testo[a++]='kdebase (KDE core applications and files)\n\
This package provides the core applications and infrastructure files\n\
for the KDE Plasma Desktop.';

testo[a++]='kdebase-runtime (KDE runtime core)\n\
This package contains core components of the KDE base runtime module.';

testo[a++]='kdebase-workspace (KDE Plasma Desktop)\n\
This package provides the essential parts of the KDE Plasma Desktop\n\
that are presented to the user.';

testo[a++]='kdebindings (KDE language bindings)\n\
...
languages, allowing them to be used to create applications for the\n\
KDE Platform.';

testo[a++]='kdeedu (Educational applications)\n\
Educational applications for the KDE Software Compilation.';

testo[a++]='kdegames (KDE games)\n\
A collection of games for the KDE Software Compilation.';

testo[a++]='kdegraphics (KDE graphics programs)\n\
Graphics programs for the KDE Software Compilation, including\n\
gwenview, kcolorchooser, kbackgroundsnapshot, kolourpaint, kruler,\n\
ksnapshot, okular, and xf86gammacfg.';

testo[a++]='kdelibs (KDE libraries)\n\
System libraries and other resources required for the KDE Platform.';

testo[a++]='kdemultimedia (KDE multimedia applications)\n\
Multimedia applications for the KDE Software Compilation, including:\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js noatun: a multimedia player for sound and movies\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js kmix: the audio mixer as a standalone program and Kicker applet\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js kscd: A CD player with an interface to the internet CDDB database\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js phonon-xine: A Phonon backend based on Xine';

testo[a++]='kdenetwork (Network utilities for KDE)\n\
Network related utilities for the KDE Software Compilation.';

testo[a++]='kdepim (KDE Personal Information Management applications)\n\
...
testo[a++]='kdepim-runtime (KDE Personal Information Management runtime)\n\
The KDE-PIM project aims to bring together those who wish to help\n\
design, implement, test, etc. anything that\'s to do with personal\n\
information management.\n\
This rather broad scope encompasses mail clients, addressbooks,\n\
...
testo[a++]='kdepimlibs (Libraries for KDE-PIM)\n\
This module includes libraries that are central to the development and\n\
execution of a KDE-PIM application.';

testo[a++]='kdeplasma-addons (Plasma addons for KDE)\n\
Plasmoids (or widgets) for the KDE Plasma Desktop shell.';

testo[a++]='kdesdk (KDE SDK)\n\
KDE Software Development Kit. This is a collection of applications\n\
and tools used to develop on the KDE Platform. It also contains\n\
example code for use in learning KDE programming or starting a new\n\
KDE application.';

testo[a++]='kdetoys (Toys for the KDE Software Compilation)\n\
Some toys for the KDE Software Compilation such as amor, kteatime,\n\
ktux, kweatherreport, and kweatherservice.';

testo[a++]='kdeutils (KDE Utilities)\n\
This package includes utilities for the KDE Software Compilation\n\
such as ark, kcalc, kcharselect, kdessh, kdf, kfloppy, kgpg,\n\
ktimer, kwalletmanager, kwikdisk, okteta, superkaramba, and\n\
sweeper.';

testo[a++]='kdevelop (development environment for KDE)\n\
The KDevelop integrated C/C++ application development environment.';

testo[a++]='kdevplatform (KDE development platform)\n\
This KDE module provides libraries used by kdevelop.';

testo[a++]='kdewebdev (web development package for the KDE Platform)\n\
The kdewebdev package contains applications which are useful for\n\
web development.';

testo[a++]='kernel-firmware (Firmware for the kernel)\n\
These are firmware files for the Linux kernel.\n\
You\'ll need these to use certain hardware drivers with Linux.\n\
Upstream site:\n\
git://git.kernel.org/pub/scm/linux/kernel/git/dwmw2/linux-firmware.git';

testo[a++]='kernel-generic (a general purpose SMP Linux kernel)\n\
...
testo[a++]='kernel-modules (Linux kernel modules)\n\
...
testo[a++]='koffice-l10n-it\n\
Italian language support for KOffice.';

testo[a++]='konq-plugins (Plugins to extend Konqueror)\n\
This module contains plugins that interact with Konqueror. Some of\n\
the plugins included are babelfish, dirfilter, domtreeviewer,\n\
htmlvalidator, cssvalidator, khtmlsettingsplugin, kimgalleryplugin,\n\
uachanger, kuick, smbmounter, autorefresh, sidebar,\n\
sidebar/mediaplayer, fsview, akregator, mediarealfolder,\n\
and minitools.';

testo[a++]='kopete-cryptography (crypto support for kopete)\n\
Cryptographic support for Kopete, the KDE Instant Messenger.';

testo[a++]='ksh (AT&T Korn Shell 93)\n\
...
testo[a++]='ktorrent (KDE BitTorrent program)\n\
...
KTorrent is released under the GPL.';

testo[a++]='lcms (little cms engine)\n\
Little cms is a small, speed optimized color management engine.\n\
lcms was written by Marti Maria marti@littlecms.com.';

testo[a++]='less (file pager)\n\
...
testo[a++]='lesstif (a free replacement for OSF/Motif(R))\n\
...
testo[a++]='lftp (command line file transfer program)\n\
...
testo[a++]='lha (LHa archive utility)\n\
...
testo[a++]='libaio (asynchronous I/O library)\n\
...
testo[a++]='libao (Audio Output library)\n\
...
testo[a++]='libarchive (archive reading library)\n\
Libarchive is a programming library that can create and read\n\
several different streaming archive formats, including most\n\
popular TAR variants and several CPIO formats. It can also\n\
write SHAR archives.\n\
Homepage: http://libarchive.googlecode.com';

testo[a++]='libart_lgpl (graphics library)\n\
...
testo[a++]='libassuan (Interprocess Communication Library for GPG)\n\
This is the IPC library used by GnuPG 2.x, GPGME and a few other\n\
packages. ';

testo[a++]='libcaca (Colour AsCii Art library)\n\
...
testo[a++]='libcap (get/set POSIX capabilities)\n\
...
testo[a++]='libcddb (An online CD database library)\n\
Libcddb is a library that implements the different protocols (CDDBP,\n\
HTTP, SMTP) to access data on a CDDB server (http://freedb.org).\n\
Visit the project at: http://libcddb.sourceforge.net/';

testo[a++]='libcdio (GNU CD access library)\n\
...
formats may benefit from using this library. A library for working\n\
with ISO-9660 filesystems libiso9660 is included. A generic interface\n\
for issuing MMC (multimedia commands) is also part of the libcdio\n\
library.';

testo[a++]='libcgroup (library and utilities for kernel control groups)\n\
...
testo[a++]='libdbusmenu-qt (Qt implementation of the DBusMenu spec)\n\
...
testo[a++]='libdiscid (library to generate DiscIDs from audio CDs)\n\
libdiscid creates MusicBrainz DiscIDs from audio CDs.\n\
It reads the table of contents (TOC) of a CD and generates\n\
an identifier which can be used to lookup the CD at MusicBrainz.\n\
Additionally, it provides a submission URL for adding the DiscID\n\
to the database.\n\
Home: http://musicbrainz.org/doc/libdiscid';
...
testo[a++]='libdrm (A library to support Direct Rendering)\n\
...
testo[a++]='libdvdread (library for reading DVDs)\n\
...
testo[a++]='libelf (ELF object file access library)\n\
Libelf lets you read, modify or create ELF files in an architecture\n\
independent way. The library takes care of size and endian issues,\n\
so for example you can process a file for SPARC processors on an\n\
Intel based system. GCC uses libelf for LTO (Link Time Optimization).\n\
Project home: http://www.mr511.de/software/';

testo[a++]='liberation-fonts-ttf (Liberation TTF Fonts)\n\
Liberation is the collective name of three TrueType font\n\
families: Liberation Sans, Liberation Serif and Liberation\n\
Mono. These fonts are metric-compatible with Arial, Times\n\
New Roman, and Courier New respectively.\n\
The fonts were developed by Steve Matteson of Ascender Corp.\n\
for Red Hat, Inc.';

testo[a++]='libexif (Exchangeable Image File Format library)\n\
...
testo[a++]='libgcrypt (General purpose crypto library)\n\
Libgcrypt is a general purpose crypto library based on the code\n\
used in GnuPG. ';

testo[a++]='libglade (GLADE library)\n\
...
testo[a++]='libgpg-error (GnuPG Error Definitions Library)\n\
...
testo[a++]='libgphoto2 (Digital Camera Library)\n\
libgphoto2 is a library that can be used by\n\
applications to access various digital cameras.\n\
Homepage: http://www.gphoto.org/';

testo[a++]='libgpod (A library for connecting to some sort of media device ;-)\n\
...
together. Just drop a mail to: Gtkpod-devel@lists.sourceforge.net\n\
(you may ask to be CC in the answer).';

testo[a++]='libgsf (structured file I/O library)\n\
libgsf aims to provide an efficient extensible I/O abstraction for\n\
dealing with different structured file formats.\n\
libgsf is used by libwv2, which is used by various word processors\n\
to import .doc format files.';

testo[a++]='libhangul (Hangul keyboard input library)\n\
...
libhangul home: http://kldp.net/projects/hangul/';

testo[a++]='libical (iCAL protocol implementation)\n\
...
testo[a++]='libid3tag (ID3 tag manipulation library)\n\
...
testo[a++]='libidn (GNU Internationalized Domain Name library)\n\
GNU Libidn is an implementation of the Stringprep, Punycode and IDNA\n\
specifications defined by the IETF Internationalized Domain Names\n\
(IDN) working group, used for internationalized domain names.';

testo[a++]='libieee1284 (parallel port library)\n\
...
testo[a++]='libiodbc (Independent Open DataBase Connectivity)\n\
...
testo[a++]='libjpeg (Independent JPEG Group\'s JPEG software)\n\
...
testo[a++]='libkarma (library for accessing the Rio Karma music player)\n\
This is a library for accessing the Rio Karma, a small media player\n\
which supports the music formats ogg vorbis, flac, mp3, wave and wma.\n\
libkarma was mostly written by Frank Zschockelt.';

testo[a++]='libksba (X.509 & CMS library for S/MIME and TLS)\n\
...
testo[a++]='libktorrent (Protocol library for the KDE BitTorrent program)\n\
Libktorrent is the BitTorrent protocol library for ktorrent, which\n\
contains all the torrent downloading code. Although it is closely\n\
to ktorrent, the goal is to make libktorrent an independent library\n\
which can be used by other applications.\n\
Homepage: http://ktorrent.org';

testo[a++]='liblastfm (last fm library)\n\
The last.fm audio scrobbling service library.';

testo[a++]='libmad (high-quality MPEG audio library)\n\
...
testo[a++]='libmcrypt (MCrypt encryption library)\n\
Libmcrypt is a data encryption library. The library is thread safe\n\
and provides encryption and decryption functions. This version of\n\
the library supports many encryption algorithms and encryption modes.\n\
Some algorithms which are supported: SERPENT, RIJNDAEL, 3DES, GOST,\n\
SAFER+, CAST-256, RC2, XTEA, 3WAY, TWOFISH, BLOWFISH, ARCFOUR, WAKE,\n\
and more.\n\
Homepage: http://mcrypt.sourceforge.net';

testo[a++]='libmcs (A library for managing configuration settings)\n\
...
many applications will adopt its use.';

testo[a++]='libmng (Multiple-image Network Graphics library)\n\
...
testo[a++]='libmowgli (A useful collection of routines for programming)\n\
libmowgli is a class library containing performance and usability\n\
oriented extensions to C.\n\
libmowgli was written by Patrick McFarland, William Pitcock,\n\
Jonathan Schleifer, Pippijn van Steenhoven, and Jilles Tjoelker.';

testo[a++]='libmpc (Multiple Precision Complex Library)\n\
...
testo[a++]='libmsn (implementation of the MSN Messenger service protocol)\n\
...
testo[a++]='libmtp (A library for MTP media devices)\n\
libmtp is a library that attempts to support Microsoft\'s Media\n\
Transfer Protocol (MTP). It was initially based on the great libptp2\n\
...
testo[a++]='libnjb (A library for Creative Nomad related media devices)\n\
...
testo[a++]='libnl (netlink socket library)\n\
...
testo[a++]='libnotify (notification library)\n\
A general library for event notification\n\
Homepage: http://www.galago-project.org/news/index.php';

testo[a++]='libogg (Ogg Bitstream library)\n\
Libogg is a library for manipulating ogg bitstreams. It handles\n\
both making ogg bitstreams and getting packets from ogg bitstreams.\n\
libogg is needed to use the Ogg Vorbis audio format.';

testo[a++]='liboggz (Ogg inspect/edit/validate tools and library)\n\
Oggz comprises liboggz and the tool oggz, which provides commands to\n\
inspect, edit and validate Ogg files. The oggz-chop tool can also be\n\
used to serve time ranges of Ogg media over HTTP by any web server\n\
that supports CGI. liboggz is a C library for reading and writing Ogg\n\
files and streams. It offers various improvements over the reference\n\
libogg, including support for seeking, validation and timestamp\n\
interpretation.';

testo[a++]='liboil (a library of simple functions)\n\
...
testo[a++]='libpcap (packet capture library)\n\
...
Project homepage: http://sourceforge.net/projects/libpcap/';

testo[a++]='libpciaccess\n\
libpciaccess is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='libplist (A library for Apple Binary and XML Property Lists)\n\
libplist is a library that attempts to support Apple Binary and XML\n\
Property Lists. It is used by libgpod.\n\
The libplist homepage: http://www.libimobiledevice.org/';

testo[a++]='libpng (Portable Network Graphics library)\n\
...
testo[a++]='libpthread-stubs\n\
libpthread-stubs is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='libraw1394 (user space IEEE 1394 driver)\n\
...
testo[a++]='librsvg (SVG library)\n\
Library for handling SVG (Scalable Vector Graphics).';

testo[a++]='libsamplerate (a Sample Rate Converter for audio)\n\
...
testo[a++]='libsndfile (C library for reading and writing wav files)\n\
Libsndfile is a C library for reading and writing files containing\n\
sampled sound (such as MS Windows WAV and Apple/SGI AIFF format).\n\
Homepage: http://www.mega-nerd.com/libsndfile/';

testo[a++]='libspectre (ghostscript wrapper library)\n\
...
testo[a++]='libtermcap (GNU termcap library)\n\
...
testo[a++]='libtheora (Theora video codec library)\n\
...
testo[a++]='libtiff (a library for reading and writing TIFF files)\n\
...
testo[a++]='libtool (a generic library support script)\n\
...
This package also contains libusb-compat for the "old" libusb users.\n\
http://libusb.org/wiki/LibusbCompat0.1';

testo[a++]='libvisual (audio visualization library)\n\
...
testo[a++]='libvisual-plugins (plugins for the libvisual library)\n\
By itself, libvisual only provides a framework to build upon. The\n\
plugins in this package are designed to work with libvisual to allow\n\
it to actually do all sorts of interesting visualizations based on\n\
metrics gathered as players process audio information data. Plugins\n\
can be mixed (more than one run at a time), or morphed from one to\n\
another. Other special effects are possible as well.\n\
Several authors have contributed libvisual-plugins under the GPL.';

testo[a++]='libvncserver (VNC Server Library)\n\
LibVNCServer makes writing a VNC server (or more correctly,\n\
a program exporting a framebuffer via the Remote Frame Buffer\n\
protocol) easy.\n\
Homepage: http://libvncserver.sourceforge.net/';

testo[a++]='libvorbis (Ogg Vorbis library)\n\
...
testo[a++]='libwmf (WMF graphics format library)\n\
...
testo[a++]='libwmf-docs (extra documentation for libwmf)\n\
This package contains extra documentation useful for programming with\n\
the libwmf (metafile graphics format) library.';

testo[a++]='libwnck (Window Navigator Construction Kit library)\n\
...
are dealing with window management. It tries hard to respect the\n\
Extended Window Manager Hints specification (EWMH).\n\
libwnck is used by compiz to provide the gtk-window-decorator program.';

testo[a++]='libwpd (WordPerfect document format library)\n\
...
testo[a++]='libx86 (hardware-independent library for x86 code execution)\n\
libx86 is a hardware-independent library for executing\n\
real-mode x86 code, such as bios calls, from userspace.\n\
Homepage: http://www.codon.org.uk/~mjg59/libx86/';

testo[a++]='libxcb (X protocol C-language Binding)\n\
...
testo[a++]='libxslt (XML transformation library)\n\
XSLT support for libxml2. (XSLT is a language used for transforming\n\
XML documents)';

testo[a++]='libzip (A library to read, create, and modify Zip archives)\n\
...
made without closing the archive can be reverted.\n\
Homepage: http://www.nih.at/libzip/';

testo[a++]='lilo (Linux Loader)\n\
...
testo[a++]='links (WWW browser for the console)\n\
...
testo[a++]='linux-faqs (Linux FAQ collection from ibiblio.org)\n\
...
and the Threads-FAQ.\n\
These FAQs will be installed under /usr/doc/Linux-FAQs/.';

testo[a++]='linux-howtos (HOWTOs from the Linux Documentation Project)\n\
Documentation on many tasks relating to Linux setup and operation.\n\
These documents will be installed in /usr/doc/Linux-HOWTOs/.';

testo[a++]='linuxdoc-tools (utilities for working with DocBook and SGML)\n\
...
HOWTO documents.\n\
This package includes everything you need to use SGML -- all of the\n\
standard SGML tools; OpenJade; tools for working with DocBook\n\
documents (various DTDs); xmlto and AsciiDoc.\n\
Refs.3pm.gz\n\
Output.3pm.gz';
...
testo[a++]='lm_sensors (hardware monitoring package)\n\
...
testo[a++]='logrotate (system log rotation tool)\n\
...
testo[a++]='loudmouth (XMPP C programming library)\n\
Loudmouth is a lightweight and easy-to-use C library for programming\n\
with the Jabber protocol. It is designed to be easy to get started\n\
with, and yet extensible enough to allow you to do anything that the\n\
Jabber protocol allows.\n\
loudmouth home: http://www.loudmouth-project.org/';

testo[a++]='lrzip (Long Range ZIP)\n\
LRZIP is a file compression program designed to do particularly well\n\
on very large files containing long distance redundancy. The larger\n\
the file and the more memory you have, the better the compression\n\
advantage this will provide. A variety of compression options allow\n\
optimizing for size or speed.\n\
Homepage: http://ck.kolivas.org/apps/lrzip/';

testo[a++]='lsof (list open files)\n\
...
testo[a++]='lsscsi (list SCSI devices or hosts, and their attributes)\n\
...
testo[a++]='lvm2 (Logical Volume Manager version 2)\n\
...
testo[a++]='lxc (Linux Containers)\n\
Linux Containers (LXC) are an operating system-level virtualization\n\
method for running multiple isolated server installs (containers) on\n\
a single control host. LXC does not provide a virtual machine, but\n\
rather provides a virtual environment that has its own process and\n\
network space. It is similar to a chroot, but offers more isolation.\n\
Daniel Lezcano is the primary developer of lxc.\n\
Homepage: http://lxc.sourceforge.net/';

testo[a++]='Lynx (text mode browser)\n\
...
testo[a++]='lzo (LZO Compression Library)\n\
LZO is a portable lossless data compression library written in ANSI C.\n\
It offers pretty fast compression and very fast decompression.\n\
Homepage: http://www.oberhumer.com/opensource/lzo/ ';

testo[a++]='m17n-lib (application multilingualization library)\n\
M17n is an abbreviation of Multilingualization. The m17n library\n\
implements multilingualization for many applications. The m17n\n\
library supports functions to handle M-texts. This package also\n\
contains the m17n database.\n\
m17n-lib home: http://m17n.org/';

testo[a++]='m4 (an implementation of the UNIX macro processor)\n\
...
testo[a++]='madplay (MPEG audio decoder and player)\n\
...
madplay was written by Robert Leslie.';

testo[a++]='mailx (a simple mail client)\n\
...
testo[a++]='make (GNU make utility to maintain groups of programs)\n\
...
testo[a++]='man (format and display the on-line manual pages)\n\
...
testo[a++]='man-pages (system documentation)\n\
...
testo[a++]='mc (Midnight Commander file manager)\n\
...
testo[a++]='mcabber (Jabber console client)\n\
mcabber is a small Jabber console client for Linux, maintained by\n\
Mikael Berthe. Mcabber includes features such as SSL support,\n\
multi-user chat (MUC), history logging, commands completion, and\n\
external actions triggers.\n\
mcabber project home is: http://www.lilotux.net/~mikael/mcabber/';

testo[a++]='mcelog (Machine Check Event logger)\n\
...
errors. It primarily handles machine checks and thermal events, which\n\
are reported for errors detected by the CPU. It is recommended that\n\
mcelog runs on all x86 machines, both 64-bit and 32bit.\n\
mcelog home: ftp://ftp.kernel.org:/pub/linux/utils/cpu/mce';

testo[a++]='mdadm (manage RAID arrays)\n\
mdadm is a utility program for creating, managing, and monitoring\n\
Linux MD (Software RAID) devices. Unlike the older raidtools\n\
package, mdadm is a single program (which should make it easier to\n\
use).\n\
mdadm was written by Neil Brown. Development of mdadm is sponsored\n\
by CSE@UNSW (The School of Computer Science and Engineering at\n\
The University of New South Wales).';

testo[a++]='mercurial (a distributed source management system)\n\
...
testo[a++]='mesa (a 3-D graphics library)\n\
...
testo[a++]='metamail (MIME extensions for mail)\n\
...
testo[a++]='mhash (Thread-safe hash library)\n\
...
testo[a++]='minicom (communications package)\n\
...
testo[a++]='mkinitrd (make an initial ramdisk)\n\
...
testo[a++]='mm (Shared Memory Allocation library)\n\
...
testo[a++]='moc (console audio player)\n\
MOC is a console audio player with a simple ncurses interface.\n\
It supports OGG, FLAC, WAV, and MP3 formats. Just run mocp, go to some\n\
directory using the menu, select a file, and press enter to play it.\n\
MOC will continue playing the rest of the files in the directory.\n\
MOC was written by Damian Pietras.\n\
Homepage: http://moc.daper.net';

testo[a++]='module-init-tools (kernel module utilities)\n\
...
testo[a++]='most (another pager, like \'more\' and \'less\')\n\
...
testo[a++]='mozilla-firefox (Mozilla Firefox Web browser)\n\
...
testo[a++]='mozilla-thunderbird (Mozilla Thunderbird mail application)\n\
...
testo[a++]='mpfr (Multiple-Precision Floating-Point Reliable Library)\n\
...
testo[a++]='mpg123 (a command-line mp3 player)\n\
mpg123 is a real time MPEG 1.0/2.0/2.5 audio player/decoder for\n\
layers 1, 2, and 3 (MPEG 1.0 layer 3 aka MP3 most commonly tested).\n\
It is free software licensed under LGPL 2.1.\n\
Homepage: http://mpg123.org/';

testo[a++]='mt-st (controls magnetic tape drive operation)\n\
...
testo[a++]='mtr (a network diagnostic tool)\n\
mtr combines the functionality of the \'traceroute\' and \'ping\' programs\n\
in a single network diagnostic tool.\n\
Matt Kimball wrote most of mtr, and mtr is currently maintained by\n\
Roger Wolff.\n\
mtr has a home page: http://www.bitwizard.nl/mtr/';

testo[a++]='mtx (Controls tape autochangers)\n\
...
testo[a++]='Mutt (the Mutt mail user agent)\n\
...
testo[a++]='mysql (SQL-based relational database server)\n\
...
testo[a++]='nano (Nano\'s ANOther editor, an enhanced free Pico clone)\n\
...
Netcat was written by *Hobbit* hobbit@avian.org, and is a product\n\
of Avian Research.';

testo[a++]='ncftp (ftp client)\n\
...
testo[a++]='ncompress (the classic *nix compression utility)\n\
...
testo[a++]='ncurses (CRT screen handling and optimization package)\n\
...
testo[a++]='neon (HTTP and WebDAV client library)\n\
neon is an HTTP and WebDAV client library, with a C language API.\n\
Bindings for other languages may also be available, see the web site\n\
for more details: http://www.webdav.org/neon/\n\
The neon library is used by projects such as subversion.';

testo[a++]='net-snmp (Simple Network Management Protocol tools)\n\
...
OID.3.gz\n\
default_store.3.gz\n\
netsnmp_request_infoPtr.3.gz\n\
TrapReceiver.3.gz\n\
agent::default_store.3.gz\n\
ASN.3.gz\n\
agent.3.gz';

testo[a++]='net-tools (base Linux networking utilities)\n\
...
testo[a++]='netatalk (Appletalk file and print server)\n\
...
testo[a++]='netdate (simple network time utility)\n\
...
testo[a++]='netkit-bootparamd (boot parameter server)\n\
...
testo[a++]='netkit-ftp (the old BSD FTP client)\n\
...
testo[a++]='netkit-ntalk (talk client and talkd daemon)\n\
...
testo[a++]='netkit-routed (dynamic network routing daemon)\n\
...
cluster of networks.\n\
Fair warning: routed is no longer maintained and may be insecure.';

testo[a++]='netket-rsh (the BSD rsh/rcp/rlogin clients and daemon)\n\
...
testo[a++]='netkit-rusers (show users logged in to machines on local network)\n\
...
testo[a++]='netkit-rwall (send a message to all users)\n\
...
testo[a++]='netkit-rwho (who is logged in on local machines)\n\
...
testo[a++]='netkit-timed (time server daemon)\n\
...
testo[a++]='netwatch (a network monitor)\n\
...
testo[a++]='network-scripts (Scripts to configure a network)\n\
...
testo[a++]='netwrite (in.writed daemon for accepting network "write")\n\
...
testo[a++]='newspost (binary posting utility)\n\
...
testo[a++]='nfs-utils (Network File System daemons and utilities)\n\
...
testo[a++]='normalize (adjusts volume levels of audio files)\n\
...
testo[a++]='notify-python (Python bindings for libnotify)\n\
The notify-python package contains bindings to use libnotify with\n\
Python programs.\n\
Homepage for notify-python: http://www.galago-project.org/';

testo[a++]='ntfs-3g (NTFS read-write filesystem driver)\n\
...
testo[a++]='ntfsprogs (utilities for working with NTFS volumes)\n\
...
testo[a++]='ntp (Network Time Protocol daemon)\n\
...
testo[a++]='obex-data-server (Obex D-Bus Service Support)\n\
Obex-data-server is a D-Bus service providing high-level OBEX client\n\
and server side functionality, and currently supports OPP, FTP\n\
profiles and Bluetooth transport.';

testo[a++]='obexfs (mount filesystem of ObexFTP capable devices)\n\
...
testo[a++]='obexftp (Access devices via ObexFTP, such as mobile phones)\n\
...
The project site is: http://triq.net/obexftp.html';
...
testo[a++]='openexr (HDR Image File Format & Library)\n\
OpenEXR is an image file format and library developed by Industrial\n\
Light & Magic, and later released to the public. It provides support\n\
for high dynamic range and a 16-bit floating point "half" data type\n\
which is compatible with the half data type in the Cg programming\n\
language.\n\
Homepage: http://www.openexr.org/';

testo[a++]='openldap-client (OpenLDAP client programs)\n\
...
testo[a++]='openobex (Object Exchange protocol library)\n\
...
Visit the project here: http://www.openobex.org';

testo[a++]='openssh (Secure Shell daemon and clients)\n\
...
testo[a++]='openssl (Secure Sockets Layer toolkit)\n\
...
testo[a++]='openssl-solibs (OpenSSL shared libraries)\n\
...
testo[a++]='openvpn (secure IP tunnel daemon)\n\
...
testo[a++]='oprofile (system profiling tool)\n\
...
testo[a++]='oxygen-icons (Oxygen theme for the KDE Plasma Desktop)\n\
Oxygen provides a complete and modern icon theme for KDE.\n\
Visit the Oxygen project online: http://www.oxygen-icons.org';

testo[a++]='p2c (Pascal to C translator)\n\
...
testo[a++]='pan (USENET newsreader)\n\
This is PAN, a powerful and user-friendly USENET newsreader based on\n\
GTK+2. Pan is the only Unix newsreader to score 100% on the Good\n\
Net-Keeping Seal of Approval newsreader evaluations.';

testo[a++]='pango (library for layout and rendering of text)\n\
...
testo[a++]='parted (GNU disk partitioning tool)\n\
...
testo[a++]='patch (apply a diff file to an original file or files)\n\
...
testo[a++]='pciutils (PCI utilities)\n\
...
testo[a++]='pcmciautils (Utilities for using PCMCIA cards with 2.6+ kernels)\n\
...
testo[a++]='pcre (Perl-compatible regular expression library)\n\
...
testo[a++]='perl (Practical Extraction and Report Language)\n\
...
StdHandle.3.gz\n\
DBD::SqlEngine.3.gz\n\
Parser::Iterator::Stream.3.gz\n\
Parser::Utils.3.gz\n\
Dist::Build::Constants.3.gz\n\
Simple::DumpAsText.3.gz\n\
Build::Platform::Amiga.3.gz\n\
Spec::VMS.3.gz\n\
Balanced.3.gz\n\
ParseWords.3.gz\n\
ExpireTest.3.gz\n\
Gofer::Policy::pedantic.3.gz\n\
Perldoc::ToRtf.3.gz\n\
Array.3.gz\n\
Text::Overstrike.3.gz\n\
SDBM_File.3.gz\n\
Dir.3.gz\n\
WithBase.3.gz\n\
Message::Item.3.gz\n\
Prove::State.3.gz\n\
DBD::SqlEngine::Developers.3.gz\n\
Hacking.3.gz\n\
Build::PPMMaker.3.gz\n\
File.3.gz\n\
Simple::Text.3.gz\n\
Maketext::Simple.3.gz\n\
Shell::Default::Plugins::Remote.3.gz\n\
PPPort.3.gz\n\
Build::Cookbook.3.gz\n\
_punycode.3.gz\n\
Peek.3.gz\n\
Build::Platform::darwin.3.gz\n\
Simple::Search.3.gz\n\
Fetch.3.gz\n\
Embed.3.gz\n\
Shell::Default::Plugins::CustomSource.3.gz\n\
Spec.3.gz\n\
Installed.3.gz\n\
Simple::LinkSection.3.gz\n\
Piece.3.gz\n\
Simple::RTF.3.gz\n\
Version.3.gz\n\
Simple::PullParserStartToken.3.gz\n\
Select.3.gz\n\
JP::JIS7.3.gz\n\
Parser::YAMLish::Writer.3.gz\n\
libnetFAQ.3.gz\n\
MakeMaker.3.gz\n\
Base.3.gz\n\
MM_Any.3.gz\n\
Parser::SourceHandler::Executable.3.gz\n\
SubstrHash.3.gz\n\
Backend::RV.3.gz\n\
stat.3.gz\n\
Fast.3.gz\n\
Build::Notes.3.gz\n\
Spec::Unix.3.gz\n\
Compress::RawDeflate.3.gz\n\
MakeMaker::Tutorial.3.gz\n\
JP.3.gz\n\
Gofer::Policy::rush.3.gz\n\
Perldoc::ToChecker.3.gz\n\
Simple::DumpAsXML.3.gz\n\
File::Roadmap.3.gz\n\
Text.3.gz\n\
Util.3.gz\n\
Collate.3.gz\n\
...
Codes::Language.3.gz\n\
JP::H2Z.3.gz\n\
Simple::PullParserToken.3.gz\n\
LangTags::Detect.3.gz\n\
gmtime.3.gz\n\
Message.3.gz\n\
Collate::CJK::Pinyin.3.gz\n\
Error.3.gz\n\
Deparse.3.gz\n\
Parser::Result.3.gz\n\
Packlist.3.gz\n\
...
Poll.3.gz\n\
Perldoc::BaseTo.3.gz\n\
MakeMaker::Config.3.gz\n\
InputObjects.3.gz\n\
Gofer::Transport::null.3.gz\n\
POP3.3.gz\n\
Internals::Search.3.gz\n\
Build::Platform::Unix.3.gz\n\
Shell::Default::Plugins::HOWTO.3.gz\n\
Codes::Changes.3.gz\n\
Maketext::GutsLoader.3.gz\n\
Split.3.gz\n\
register.3.gz\n\
Kwalify.3.gz\n\
Harness.3.gz\n\
Xref.3.gz\n\
Build::Platform::cygwin.3.gz\n\
AnyDBM_File.3.gz\n\
HiRes.3.gz\n\
Parser::Scheduler::Job.3.gz\n\
Pluggable.3.gz\n\
Semaphore.3.gz\n\
Dist::Autobundle.3.gz\n\
Internals::Utils.3.gz\n\
Config.3.gz\n\
Meta::YAML.3.gz\n\
Spec::Functions.3.gz\n\
DBD::SqlEngine::HowTo.3.gz\n\
Build::Platform::EBCDIC.3.gz\n\
Simple::HTML.3.gz\n\
Cookie.3.gz\n\
Unicode::UTF7.3.gz\n\
Typemap.3.gz\n\
...
Copy.3.gz\n\
URL.3.gz\n\
Simple::TextContent.3.gz\n\
KR.3.gz\n\
Util::FieldHash.3.gz\n\
BigFloat.3.gz\n\
Load.3.gz\n\
Parser::Result::Bailout.3.gz\n\
MM.3.gz\n\
Meta::Converter.3.gz\n\
Const::GetInfoType.3.gz\n\
Lint::Debug.3.gz\n\
Symbol.3.gz\n\
Switch.3.gz\n\
Parser::Multiplexer.3.gz\n\
ProxyServer.3.gz\n\
Currency.3.gz\n\
Parser::Iterator::Process.3.gz\n\
KR::2022_KR.3.gz\n\
DosGlob.3.gz\n\
Lint.3.gz\n\
via::QuotedPrint.3.gz\n\
Build::ModuleInfo.3.gz\n\
Terse.3.gz\n\
netent.3.gz\n\
Domain.3.gz\n\
Trig.3.gz\n\
...
Wrap.3.gz\n\
Constant::Utils.3.gz\n\
Simple::HTMLBatch.3.gz\n\
Perldoc::GetOptsOO.3.gz\n\
Build::Version.3.gz\n\
CJKConstants.3.gz\n\
Build::Platform::aix.3.gz\n\
Parser::Result::Test.3.gz\n\
FTP.3.gz\n\
Gofer::Transport::stream.3.gz\n\
GSM0338.3.gz\n\
RefHash.3.gz\n\
Build::Authoring.3.gz\n\
Gofer.3.gz\n\
Compress::Bzip2.3.gz\n\
Cmd.3.gz\n\
Proxy.3.gz\n\
...
tm.3.gz\n\
BigRat.3.gz\n\
Pluggable::Object.3.gz\n\
Simple::PullParser.3.gz\n\
DBD::Metadata.3.gz\n\
compress.3.gz\n\
Build::Base.3.gz\n\
DBD::mysql.3.gz\n\
...
Meta.3.gz\n\
Nox.3.gz\n\
Parser::Scheduler::Spinner.3.gz\n\
Parser::Iterator.3.gz\n\
Simple.3.gz\n\
Parser.3.gz\n\
Cap.3.gz\n\
PurePerl.3.gz\n\
...
Queue.3.gz\n\
SMTP.3.gz\n\
Parser::SourceHandler::File.3.gz\n\
Simple::Subclassing.3.gz\n\
Complete.3.gz\n\
MM_OS2.3.gz\n\
Constant::XS.3.gz\n\
grent.3.gz\n\
Parser::Style::Stream.3.gz\n\
Expire.3.gz\n\
utf8.3.gz\n\
Internals::Fetch.3.gz\n\
ParseXS.3.gz\n\
Const::GetInfoReturn.3.gz\n\
API::HOWTO.3.gz\n\
Prove.3.gz\n\
Syslog.3.gz\n\
File.3.gz\n\
...
Metadata.3.gz\n\
file.3.gz\n\
Formatter::Console::Session.3.gz\n\
Raw::Bzip2.3.gz\n\
Perldoc::ToPod.3.gz\n\
XSSymSet.3.gz\n\
UI::History.3.gz\n\
shared.3.gz\n\
Parser::Result::Plan.3.gz\n\
Mksymlists.3.gz\n\
Byte.3.gz\n\
Hash.3.gz\n\
MM_UWIN.3.gz\n\
...
QuotedPrint.3.gz\n\
MY.3.gz\n\
Build::Platform::MPEiX.3.gz\n\
Tar.3.gz\n\
Distroprefs.3.gz\n\
Formatter::Console.3.gz\n\
Perldoc::ToMan.3.gz\n\
Gofer::Serializer::Storable.3.gz\n\
Const::GetInfo::ANSI.3.gz\n\
Uncompress::AnyInflate.3.gz\n\
Handlers.3.gz\n\
Encoder.3.gz\n\
CheckTree.3.gz\n\
MM_Unix.3.gz\n\
Perldoc::ToTk.3.gz\n\
Parser::Style::Tree.3.gz\n\
Dist::MM.3.gz\n\
Gofer::Serializer::DataDumper.3.gz\n\
Gofer::Serializer::Base.3.gz\n\
Perldoc::ToText.3.gz\n\
MD5.3.gz\n\
ldap.3.gz\n\
Compress::Gzip.3.gz\n\
...
Codes::Country.3.gz\n\
Object.3.gz\n\
Build::Platform::Windows.3.gz\n\
Internals::Extract.3.gz\n\
Perldoc.3.gz\n\
OSType.3.gz\n\
null.3.gz\n\
FAQ.3.gz\n\
Parser::Style::Objects.3.gz\n\
Tabs.3.gz\n\
Prove::State::Result.3.gz\n\
Simple::XHTML.3.gz\n\
MM_MacOS.3.gz\n\
Internals::Source.3.gz\n\
data.3.gz\n\
Builder::Module.3.gz\n\
scalar.3.gz\n\
FAQ.3.gz\n\
...
Escapes.3.gz\n\
Escape.3.gz\n\
Perldoc::ToNroff.3.gz\n\
Maketext::Cookbook.3.gz\n\
Util.3.gz\n\
SHA.3.gz\n\
servent.3.gz\n\
Collate::CJK::Korean.3.gz\n\
Module::Author.3.gz\n\
Open2.3.gz\n\
Codes::Currency.3.gz\n\
Miniperl.3.gz\n\
Push.3.gz\n\
Uncompress::Bunzip2.3.gz\n\
Country.3.gz\n\
Time.3.gz\n\
Builder::Tester.3.gz\n\
CoreList.3.gz\n\
...
BigInt::CalcEmu.3.gz\n\
ProfileDumper.3.gz\n\
Internals::Source::SQLite.3.gz\n\
Parser::Result::Unknown.3.gz\n\
Uncompress::Base.3.gz\n\
Compress::Base.3.gz\n\
Checker.3.gz\n\
Gofer::Transport::stream.3.gz\n\
DBIODBC.3.gz\n\
...
Extract.3.gz\n\
ExpireFile.3.gz\n\
Gofer::Policy::Base.3.gz\n\
Config.3.gz\n\
Build::Bundling.3.gz\n\
NNTP.3.gz\n\
Uncompress::Inflate.3.gz\n\
Install.3.gz\n\
LangTags.3.gz\n\
NDBM_File.3.gz\n\
Formatter::Session.3.gz\n\
Util::XS.3.gz\n\
Parser::Result::Comment.3.gz\n\
Simple::PullParserEndToken.3.gz\n\
TW.3.gz\n\
Semaphore.3.gz\n\
Collate::CJK::Big5.3.gz\n\
Maketext.3.gz\n\
hostent.3.gz\n\
Message::Simple.3.gz\n\
Tarzip.3.gz\n\
Parser.3.gz\n\
Std.3.gz\n\
Gofer::Policy::classic.3.gz\n\
Handle.3.gz\n\
Constants.3.gz\n\
MM_VOS.3.gz\n\
File::Developers.3.gz\n\
Cpan.3.gz\n\
InnerPackage.3.gz\n\
protoent.3.gz\n\
Language.3.gz\n\
Pipe.3.gz\n\
Dumper.3.gz\n\
Simple::PullParserTextToken.3.gz\n\
Uncompress::RawInflate.3.gz\n\
Gofer::Response.3.gz\n\
Text::Termcap.3.gz\n\
Maketext::TPJ13.3.gz\n\
Gofer::Execute.3.gz\n\
...
Spec::OS2.3.gz\n\
W32ODBC.3.gz\n\
Parser::Aggregator.3.gz\n\
Build::Platform::Default.3.gz\n\
Seekable.3.gz\n\
Formatter::Base.3.gz\n\
Spec::Cygwin.3.gz\n\
pwent.3.gz\n\
...
testlib.3.gz\n\
DBD.3.gz\n\
Collate::CJK::Stroke.3.gz\n\
exception.3.gz\n\
CBuilder.3.gz\n\
MM_DOS.3.gz\n\
SysV.3.gz\n\
Check.3.gz\n\
BigInt::FastCalc.3.gz\n\
Normalize.3.gz\n\
Config.3.gz\n\
Man.3.gz\n\
Parser::Grammar.3.gz\n\
Collate::CJK::GB2312.3.gz\n\
Base64.3.gz\n\
Parser::Result::Version.3.gz\n\
Build::Platform::RiscOS.3.gz\n\
Shell::Default::Plugins::Source.3.gz\n\
ParseLink.3.gz\n\
Formatter::File.3.gz\n\
Build::ConfigData.3.gz\n\
Tar::File.3.gz\n\
Parser::Source.3.gz\n\
Dist::Base.3.gz\n\
Parser::Scheduler.3.gz\n\
Parser::SourceHandler::RawTAP.3.gz\n\
Hash::NamedCapture.3.gz\n\
FirstTime.3.gz\n\
Langinfo.3.gz\n\
Parser::SourceHandler.3.gz\n\
Gofer::Transport::pipeone.3.gz\n\
Meta::Feature.3.gz\n\
encoding.3.gz\n\
Compare.3.gz\n\
Builder.3.gz\n\
Socket::UNIX.3.gz\n\
Uncompress::AnyUncompress.3.gz\n\
Netrc.3.gz\n\
Parser::SourceHandler::Handle.3.gz\n\
Const::GetInfo::ODBC.3.gz\n\
Module::Checksums.3.gz\n\
warnings.3.gz\n\
MM_Cygwin.3.gz\n\
File.3.gz\n\
Gofer::Request.3.gz\n\
Util::CacheMemory.3.gz\n\
Extensions.3.gz\n\
Gofer::Transport::Base.3.gz\n\
Build::Platform::MacOS.3.gz\n\
Build::Platform::os2.3.gz\n\
Alias.3.gz\n\
Spec::Mac.3.gz\n\
Spec::Epoc.3.gz\n\
DProf.3.gz\n\
...
Complex.3.gz\n\
Util::Call.3.gz\n\
ProfileDumper::Apache.3.gz\n\
Internals::Source::Memory.3.gz\n\
Compress::Zip.3.gz\n\
Module::Author::Fake.3.gz\n\
Build::API.3.gz\n\
Meta::Prereqs.3.gz\n\
QueryParam.3.gz\n\
Select.3.gz\n\
Parser::Style::Subs.3.gz\n\
CPAN::Meta.3.gz\n\
Debug.3.gz\n\
Simple.3.gz\n\
Builder::Tester::Color.3.gz\n\
Formatter::Color.3.gz\n\
mysql.3.gz\n\
PP::Boolean.3.gz\n\
Scalar.3.gz\n\
Carp.3.gz\n\
...
HandleConfig.3.gz\n\
Temp.3.gz\n\
Formatter::Console::ParallelSession.3.gz\n\
Mkbootstrap.3.gz\n\
Parser::Iterator::Array.3.gz\n\
Memoize.3.gz\n\
Local.3.gz\n\
Pretty.3.gz\n\
Heavy.3.gz\n\
Uncompress::Gunzip.3.gz\n\
Manifest.3.gz\n\
Formatter::File::Session.3.gz\n\
Command::MM.3.gz\n\
Text::Color.3.gz\n\
Simple::Checker.3.gz\n\
Parser::Style::Debug.3.gz\n\
CN.3.gz\n\
Load::Conditional.3.gz\n\
Gofer::Transport::corostream.3.gz\n\
Script.3.gz\n\
Internals.3.gz\n\
Shell::Classic.3.gz\n\
Gofer::Transport::Base.3.gz\n\
...
Handle.3.gz\n\
Simple.3.gz\n\
Simple::XMLOutStream.3.gz\n\
UCD.3.gz\n\
GlobMapper.3.gz\n\
Build::Platform::VOS.3.gz\n\
Unicode.3.gz\n\
Html.3.gz\n\
EBCDIC.3.gz\n\
ANSIColor.3.gz\n\
Simple::Methody.3.gz\n\
UI.3.gz\n\
Compress::Deflate.3.gz\n\
Module::Fake.3.gz\n\
Gofer::Transport::pipeone.3.gz\n\
Parser::SourceHandler::Perl.3.gz\n\
Util.3.gz\n\
Usage.3.gz\n\
Sponge.3.gz\n\
Accessor.3.gz\n\
...
MM_VMS.3.gz\n\
Zlib.3.gz\n\
Collate::CJK::JISX0208.3.gz\n\
Queue.3.gz\n\
Profile.3.gz\n\
Parser::IteratorFactory.3.gz\n\
Internals.3.gz\n\
encode.3.gz\n\
Showlex.3.gz\n\
Meta::History.3.gz\n\
Prove::State::Result::Test.3.gz\n\
Parser::Result::Pragma.3.gz\n\
Meta::Spec.3.gz\n\
Configure.3.gz\n\
Soundex.3.gz\n\
Internals::Report.3.gz\n\
SQL::Nano.3.gz\n\
Constant::Base.3.gz\n\
LaTeX.3.gz\n\
Uncompress::Unzip.3.gz\n\
Message::Config.3.gz\n\
Build::Platform::VMS.3.gz\n\
localtime.3.gz\n\
PP.3.gz\n\
Parser::ResultFactory.3.gz\n\
Hostname.3.gz';

testo[a++]='phonon (multimedia framework for KDE4)\n\
Phonon is the multimedia framework for KDE4. This package contains\n\
the xine and gstreamer backends and support files for KDE. The\n\
Phonon library itself is included with Qt.';

testo[a++]='phonon-mplayer (mplayer backend for phonon)\n\
This package provides an additional mplayer backend for the phonon\n\
multimedia framework used in KDE.\n\
Home: http://code.google.com/p/phonon-vlc-mplayer/';

testo[a++]='php (HTML-embedded scripting language)\n\
...
testo[a++]='pidentd (TCP/IP IDENT protocol server)\n\
...
testo[a++]='pidgin (GTK+ instant messaging program)\n\
...
testo[a++]='pil (Python Imaging Library)\n\
...
testo[a++]='pilot-link (library for communicating with PalmOS devices)\n\
...
testo[a++]='pinentry (PIN Entry dialogs)\n\
This is a collection of simple PIN or passphrase entry dialogs which\n\
utilize the Assuan protocol as described by the aegypten project.\n\
See http://www.gnupg.org/aegypten/ for details.';

testo[a++]='pixman\n\
pixman contains low-level pixel manipulation routines and is used by\n\
both xorg and cairo.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='pkg-config (system for managing library compile/link flags)\n\
...
testo[a++]='pkgtools (The Slackware package maintenance system)\n\
...
testo[a++]='pm-utils (Power Management Utilities)\n\
The pm-utils package provides simple shell command line tools to\n\
suspend and hibernate computers that can be used to run vendor or\n\
distribution supplied scripts on suspend and resume.\n\
Homepage: http://pm-utils.freedesktop.org/';

testo[a++]='pmake (BSD parallel make)\n\
...
testo[a++]='polkit (authentication framework)\n\
...
Home: http://www.freedesktop.org/wiki/Software/PolicyKit';

testo[a++]='polkit-gnome (GTK+ authentication agent for polkit)\n\
polkit-gnome provides a GTK+ based authentication agent for polkit.\n\
It can be used by any desktop environment that follows the XDG\n\
Base Directory Specification.';

testo[a++]='polkit-kde-1 (KDE polkit authenticator)\n\
This is a polkit authenticator for KDE.';

testo[a++]='polkit-qt-1 (Qt polkit API wrapper)\n\
polkit-qt-1 aims to make it easy for Qt developers to take advantage\n\
of the polkit API. It is a convenience wrapper around QAction and\n\
QAbstractButton that lets you integrate those two components\n\
easily with polkit.\n\
Homepage: http://techbase.kde.org/Polkit-Qt-1';

testo[a++]='popa3d (a POP3 daemon)\n\
...
testo[a++]='poppler (a library for rendering PDF documents)\n\
...
testo[a++]='poppler-data (data files used by poppler)\n\
...
testo[a++]='popt (command line parsing library)\n\
...
testo[a++]='portmap (a daemon to manage RPC connections)\n\
...
testo[a++]='powertop (tool for analyzing system power usage)\n\
...
testo[a++]='ppp (Point-to-Point Protocol)\n\
...
testo[a++]='procmail (mail processing and local delivery program)\n\
...
testo[a++]='procps (utilities for displaying process information)\n\
...
testo[a++]='proftpd (FTP server daemon)\n\
...
testo[a++]='pssh (tools for collective remote system management)\n\
Pssh, a.k.a. "parallel ssh", are Python-written wrapper scripts for\n\
OpenSSH and rsync tools. They are useful for simultaneous control\n\
of large number of machines, using parallel ssh sessions. Included\n\
are parallel versions of ssh, scp, and rsync, as well as a parallel\n\
kill command.\n\
Pssh has been written by Brent Chun bnc at theether.org, and is\n\
currently maintained by Andrew McNabb amcnabb at mcnabbs.org.\n\
Homepage: http://parallel-ssh.googlecode.com';

testo[a++]='pth (GNU Portable Threads)\n\
...
testo[a++]='pycairo (a set of Python bindings for the cairo graphics library)\n\
Pycairo is set of Python bindings for the cairo graphics library.\n\
Pycairo\'s web site: http://cairographics.org/pycairo';

testo[a++]='pycups (Python bindings for the CUPS API)\n\
These Python bindings are intended to wrap the CUPS API.\n\
Homepage: http://cyberelk.net/tim/software/pycups/';

testo[a++]='pygobject (GObject bindings for Python)\n\
...
testo[a++]='pygtk (GTK+ bindings for Python)\n\
PyGTK provides a convenient wrapper for the GTK+ library for use in\n\
Python programs, taking care of many of the boring details such as\n\
managing memory and type casting.\n\
PyGTK\'s web page: http://www.pygtk.org/';

testo[a++]='pyrex (A Python extension modules language)\n\
Pyrex is a language for writing Python extension modules.\n\
Pyrex lets you write code that mixes Python and C data types any\n\
way you want, and then compiles it into a C extension for Python.\n\
The home page for Pyrex is:\n\
http://www.cosc.canterbury.ac.nz/~greg/python/Pyrex/';

testo[a++]='python (object-oriented interpreted programming language)\n\
...
testo[a++]='qca (Qt Cryptographic Architecture)\n\
...
testo[a++]='qca-cyrus-sasl (Cyrus SASL plugin for QCA)\n\
This plugin provides features based on Cyrus SASL version 2.';

testo[a++]='qca-gnupg (GnuPG plugin for QCA)\n\
This plugin provides features based on GnuPG.';

testo[a++]='qca-ossl (OpenSSL plugin for QCA)\n\
This plugin provides features based on OpenSSL. It implements:\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Hashing - SHA1, SHA0, RIPEMD160, MD2, MD4, MD5\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Hashing - SHA224, SHA256, SHA384 and SHA512\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Block Ciphers\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Keyed Hash Message Authentication Code (HMAC), using SHA1,\n\
MD5, and RIPEMD160\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js Public keys - RSA, DSA, Diffie-Hellman\n\
backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js PKCS#12 backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js SSL/TLS backup corsi fortune t-commedia.js t-dattilotest.js t-fortune-all.js t-fortune-all.js.bck t-lezioni.js t-linux.js t-packages.js t-random.js t-system.js CMS (for S/MIME)';

testo[a++]='QImageBlitz (Graphical effect and filter library for KDE4)\n\
Blitz is a graphical effect and filter library for KDE4.0 that\n\
contains many improvements over KDE 3.x\'s kdefx library\n\
including bugfixes, memory and speed improvements, and MMX/SSE\n\
support.';

testo[a++]='Qt (a multi-platform C++ graphical user interface toolkit)\n\
...
testo[a++]='qtscriptgenerator (Qt API Bindings for QtScript)\n\
...
testo[a++]='quota (Linux disk quota utilities)\n\
...
testo[a++]='radeon_ucode (microcode/firmware for some Radeon graphics cards)\n\
radeon_ucode contains the microcode/firmware for some models of\n\
ATI/AMD Radeon graphics cards.\n\
http://people.freedesktop.org/~agd5f/radeon_ucode/';

testo[a++]='radeontool (small utility for ati radeon-based laptops)\n\
Radeontool is a small utility to control ATI Radeon-based\n\
laptops\' backlight and external output functions\n\
...
testo[a++]='raptor (RDF Parser & Serializer)\n\
...
testo[a++]='rasqal (RDF parsing library)\n\
...
testo[a++]='rcs (Revision Control System)\n\
...
testo[a++]='rdesktop (Remote Desktop Protocol client)\n\
...
For more information, see: http://www.rdesktop.org';

testo[a++]='rdist (remote file distribution program)\n\
...
testo[a++]='readline (line input library with editing features)\n\
...
testo[a++]='recordmydesktop (record desktop sessions with audio and video)\n\
recordMyDesktop is a desktop session recorder for GNU/Linux\n\
that attemps to be easy to use, yet also effective at it\'s\n\
primary task.\n\
The command-line program (the backend) and the graphical\n\
interface (the frontend) are separate programs, but all the\n\
functionality relies on the backend which is also a standalone\n\
program. The frontend can\'t function on it\'s own.\n\
recordmydesktop home: http://recordmydesktop.sourceforge.net/';
...
testo[a++]='redland (RDF high-level interface library)\n\
...
testo[a++]='reiserfsprogs (Reiserfs filesystem utilities)\n\
...
/usr/doc/reiserfsprogs-3.6.21/README for full credits.';
...
testo[a++]='resourceproto\n\
resourceproto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='rexima (console audio mixer)\n\
Rexima is a curses-based interactive audio mixer. It can also be used\n\
from the command-line. It\'s designed to be small, fast, and\n\
straightforward. It also offers a command line mode so you can script\n\
mixer settings.\n\
Rexima was written by Russell Marks.';

testo[a++]='rfkill (tool to query subsystem interfaces)\n\
...
testo[a++]='rp-pppoe (Roaring Penguin PPPoE client)\n\
...
testo[a++]='rpm (RPM package format tool)\n\
...
testo[a++]='rpm2tgz (a tool for converting an RPM archive into a tar+gz one)\n\
...
testo[a++]='rsync (remote file sync)\n\
...
testo[a++]='rt2860-firmware (RT28XX/RT30XX PCI/mPCI/PCIe/CardBus firmware)\n\
This is firmware for PCI, mPCI, PCIe, and Cardbus wireless cards\n\
using the Ralink RT2760, RT2790, RT2860, RT2890, RT3060, RT3062,\n\
RT3562, RT2860, RT2760, RT2890, RT2790, and RT3090 wireless chipsets.';

testo[a++]='rt2870-firmware (RT28XX/RT30XX USB firmware)\n\
This is firmware for USB wireless devices using Ralink RT2870, RT2770,\n\
RT3572, and RT3070 wireless chipsets.';

testo[a++]='rt61-firmware (Firmware for Ralink rt61 wireless chips)\n\
This is firmware for the Ralink rt61 wireless chip.';

testo[a++]='rt71w-firmware (Firmware for Ralink rt71w wireless chips)\n\
This is firmware for the Ralink rt71w wireless chip.';

testo[a++]='ruby (Interpreted object-oriented scripting language)\n\
...
testo[a++]='rxvt (terminal emulator)\n\
...
testo[a++]='rzip (a large-file compression program)\n\
...
testo[a++]='samba (SMB/CIFS file and print server)\n\
Samba is an SMB/CIFS file and print server for SMB and CIFS clients.\n\
It allows you to make file space or printers on a Samba host\n\
available to SMB/CIFS clients (such as PCs running Windows).\n\
...
testo[a++]='sane (Scanner Access Now Easy)\n\
...
testo[a++]='sazanami-fonts-ttf (Japanese TrueType Unicode fonts)\n\
The Sazanami fonts contain Hiragana, Katakana, and Kanji/Han\n\
Ideographs.\n\
Homepage of efont: http://sourceforge.jp/projects/efont/';

testo[a++]='sc (spreadsheet calculator)\n\
This is a simple curses-based spreadsheet program.';

testo[a++]='scim (Smart Common Input Method platform)\n\
...
scim home: http://www.scim-im.org/';

testo[a++]='scim-anthy (a SCIM IMEngine module for anthy)\n\
SCIM is Smart Common Input Method platform, it provides a user\n\
friendly and full featured user interface and development platform to\n\
make input method developer\'s lives easier. Anthy is a system for\n\
Japanese input method. It converts Hiragana text to Kana Kanji\n\
mixed text.\n\
scim-anthy home: http://scim-imengine.sourceforge.jp/';

testo[a++]='scim-bridge (yet another gtk-immodule for SCIM)\n\
Scim-bridge is yet another gtk-immodule for SCIM. Scim-bridge\n\
consists of two parts, that is, client and agent (server). The client\n\
is a GTK immodule which communicates with the agent over sockets.\n\
The agent communicates with SCIM, and brings back the data\n\
for each client.\n\
scim-bridge home: http://scim-im.org/';

testo[a++]='scim-hangul (SCIM IMengine for Hangul)\n\
Scim-hangul is a SCIM IMEngine module for inputing korean scripts.\n\
Scim-hangul is for native speakers, not for foreigners.\n\
If you don\'t want to learn korean keyboard layout, use scim-tables.\n\
This engine supports 2 beolsik, 3 beolsik final, 3 beolsik 390,\n\
3 beolsik, no-shift and etc. You can input chinese characters\n\
(hanja) with F9 key when you a have preedit string.\n\
scim-hangul home: http://www.scim-im.org/projects/imengines';

testo[a++]='scim-input-pad (on-screen input pad for scim)\n\
This is scim-input-pad, which provides an on-screen input pad\n\
to enter symbols and chinese characters.\n\
scim home: http://scim-im.org/';

testo[a++]='scim-m17n (bridge IMengine to support m17n input methods in SCIM)\n\
SCIM is a developing platform to significantly reduce the difficulty\n\
of input method development. scim-m17n is a SCIM IMEngine module\n\
which uses the m17n library as the backend.\n\
scim-m17n home: http://scim-im.org/';

testo[a++]='scim-pinyin (smart pinyin IM engine)\n\
This package provides a smart pinyin IM engine for SCIM. It includes\n\
the plugin modules and the data files to use the smart pinyin input\n\
input method for Chinese.\n\
scim-pinyin home: http://www.scim-im.org/';

testo[a++]='scim-tables (tables IMEngine for scim)\n\
This is scim-tables IMEngine, which provides many CJK input methods.\n\
scim-tables home: http://scim-im.org/';

testo[a++]='screen (screen manager with VT100/ANSI terminal emulation)\n\
...
testo[a++]='scrnsaverproto\n\
scrnsaverproto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='sdl (Simple DirectMedia Layer library)\n\
...
testo[a++]='sdparm (fetch and change SCSI attributes)\n\
...
a SCSI command set. Apart from SCSI disks, examples of devices that\n\
use SCSI command sets are ATAPI CD/DVD drives, SCSI and ATAPI tape\n\
drives and SCSI enclosures.\n\
sdparm was written by Douglas Gilbert.';

testo[a++]='SeaMonkey (an open-source web browser suite)\n\
...
testo[a++]='seamonkey-solibs (Shared libraries from Seamonkey)\n\
...
testo[a++]='seejpeg (console JPEG viewer)\n\
...
testo[a++]='sendmail (mail transfer agent)\n\
...
testo[a++]='sendmail-cf (configuration files for sendmail)\n\
These files are used to create sendmail.cf configuration files. The\n\
m4 macro processor is also required in order to make use of these\n\
files.\n\
The files and documentation in /usr/share/sendmail should make it\n\
...
testo[a++]='seyon (telecommunications package)\n\
...
testo[a++]='shadow (shadow password suite)\n\
...
testo[a++]='shared-desktop-ontologies (Shared OSCAF desktop ontologies)\n\
...
testo[a++]='shared-mime-info (MIME database)\n\
...
testo[a++]='sharutils (GNU shell archive packing utilities)\n\
...
testo[a++]='sinhala_lklug-font-ttf (Sinhala Unicode TrueType font)\n\
This is a free Sri Lankan \'Sinhala\' Unicode font. Its development was\n\
initiated by the LK LUG in 2003.\n\
Development home: http://sinhala.sourceforge.net/';

testo[a++]='SIP (a tool for generating Python bindings)\n\
...
testo[a++]='skanlite (a scanning program)\n\
Skanlite is a light-weight scanning application based on libksane.';

testo[a++]='slackpkg+sl plugin for slacky.eu repository\n\
Slackpkg is a package manager for Slackware.\n\
Slackpkg+sl is a plugin for slackpkg\n\
It add support for slacky.eu repository.';

testo[a++]='slackpkg\n\
Slackpkg is a package manager for Slackware. It can download,\n\
upgrade, install and remove selected packages. With it, you\n\
can search in the Slackware Package database and find which\n\
package has the selected file.\n\
Slackpkg DOESN\'T replace pkgtool. Actually, Slackpkg uses pkgtool\n\
for all install|upgrade|remove functions.';

testo[a++]='slang (S-Lang interpreter version 2)\n\
...
testo[a++]='slang (S-Lang interpreter version 1)\n\
S-Lang is an interpreted language that was designed from the start\n\
to be easily embedded into a program to provide it with a powerful\n\
extension language. S-Lang is also a programmer\'s library that\n\
permits a programmer to develop sophisticated platform-independent\n\
...
slrn was written and is maintained by John E. Davis.';

testo[a++]='smartmontools (hard drive monitoring utilities)\n\
...
testo[a++]='snownews (a console RSS newsreader)\n\
Snownews is a console RSS/RDF news reader. It supports all versions\n\
of RSS natively and can be expanded via plugins to support many other\n\
other formats.\n\
Snownews was written and is maintained by Oliver Feiler.\n\
Snownews homepage: http://kiza.kcore.de/software/snownews';

testo[a++]='Soma - a command line/dialog internet radio player\n\
Soma is a front end for mplayer to play radio playlists pls,m3u and\n\
ram. More will be added as it progresses.\n\
Homepage: http://www.dawoodfall.net/slackbuilds/13.0/soma';

testo[a++]='soprano (C++/Qt4 framework for RDF data)\n\
Soprano (formally known as QRDF) is a library which provides\n\
a nice Qt interface to RDF storage solutions. It has a modular\n\
structure which allows to replace the actual RDF storage\n\
implementation used.\n\
Homepage: http://soprano.sourceforge.net/';

testo[a++]='sox (universal sound sample translator)\n\
...
testo[a++]='splitvt (run multiple shells in a single window)\n\
...
testo[a++]='SQLite (simple, self contained database engine)\n\
...
testo[a++]='startup-notification ("busy" cursors support)\n\
Adds support for applications to use "busy" cursors while starting up.';

testo[a++]='startup-notification ("busy" cursors support)\n\
Adds support for applications to use "busy" cursors while starting up.';

testo[a++]='strace (trace system calls and signals)\n\
...
testo[a++]='strigi (fast and light desktop search engine)\n\
...
testo[a++]='stunnel (Universal SSL tunnel)\n\
...
testo[a++]='subversion (a version control system)\n\
...
testo[a++]='sudo (give limited root privileges to certain users)\n\
...
testo[a++]='svgalib (A low level graphics library for Linux)\n\
...
testo[a++]='swig (Simplified Wrapper and Interface Generator)\n\
...
testo[a++]='sysfsutils (Utilities for the sysfs filesystem)\n\
...
testo[a++]='sysklogd (Linux system logging utilities)\n\
...
testo[a++]='syslinux (SYSLINUX/PXELINUX/ISOLINUX boot loaders)\n\
...
testo[a++]='sysstat (System performance monitoring tools)\n\
The sysstat utilities are a collection of performance monitoring\n\
tools for Linux. These include the iostat, mpstat, pidstat, sadf,\n\
and sar tools.\n\
The sysstat utilities were written by Sebastien Godard.\n\
For more information, see the sysstat web site:\n\
http://pagesperso-orange.fr/sebastien.godard/';

testo[a++]='system-config-printer (CUPS printer administration tool)\n\
...
Home: http://cyberelk.net/tim/software/system-config-printer/';

testo[a++]='sysvinit (init, the parent of all processes)\n\
...
testo[a++]='sysvinit-functions (/etc/init.d/functions)\n\
...
testo[a++]='sysvinit-scripts (the basic scripts used to boot your machine)\n\
These are the Slackware boot scripts, which are needed to start the\n\
machine. Sysvinit looks for these in /etc/rc.d/.';

testo[a++]='t1lib (Type 1 font rasterizer library)\n\
...
testo[a++]='taglib (audio meta-data library)\n\
TagLib is a library for reading and editing the meta-data of several\n\
popular audio formats. Currently it supports both ID3v1 and ID3v2\n\
for MP3 files, Ogg Vorbis comments and ID3 tags and Vorbis comments\n\
in FLAC files.';

testo[a++]='taglib-extras (taglib support for other formats)\n\
Taglib file type plugins which were split off from Amarok\n\
to allow others to use and help maintain them.\n\
Taglib-extras delivers support for reading and editing\n\
meta-data of audio formats not supported by taglib, including:\n\
asf, mp4v2, rmff, wav.\n\
The taglib-extras are part of kdesupport: http://www.kde.org';

testo[a++]='Tango icon theme (Another standard icon theme)\n\
The Tango Desktop Project has designed a basic set of desktop icons\n\
that follow the style guidelines. The set has been created as a\n\
proof of concept for the style, but works rather well as a replacement\n\
for the base theme under GNOME and KDE.\n\
Tango\'s home page is: http://tango.freedesktop.org';

testo[a++]='tango-icon-theme-extras (Extras for the Tango icon theme)\n\
This is an extension to the Tango Icon Theme. Currently it includes\n\
Tango icons for iPod Digital Audio Player (DAP) devices and the Dell\n\
Pocket DJ DAP.\n\
Visit the Tango project at: http://tango-project.org/';

testo[a++]='tar (archiving utility)\n\
...
testo[a++]='tcl (Tool Command Language)\n\
...
testo[a++]='tcp_wrappers (TCP/IP daemon wrapper library and utilities)\n\
...
testo[a++]='tcpdump (network monitoring tool)\n\
...
Project homepage: http://sourceforge.net/projects/tcpdump/';

testo[a++]='tcsh (C shell)\n\
...
testo[a++]='telnet (the telnet client and daemon)\n\
...
testo[a++]='tetex (teTeX binaries and support files)\n\
...
testo[a++]='tetex-doc (Documentation for teTeX)\n\
...
for the TeX Directory Standard (TDS), The TeX and LaTeX Catalogue,\n\
TeX FAQs, and more.';

testo[a++]='texinfo (GNU software documentation system)\n\
...
testo[a++]='tftp-hpa (Trivial File Transfer Protocol)\n\
...
testo[a++]='thunar-volman (Thunar Volume Manager Plugin)\n\
thunar-volman is an extension for the Thunar File Manager,\n\
which enables automatic management of removable drives and\n\
media.\n\
It was designed to look and act similar to gnome-volume-manager\n\
to get consistent removable drive and media management in both\n\
Xfce and GNOME. ';

testo[a++]='tibmachuni-font-ttf (Tibetan Machine Unicode font)\n\
The Tibetan & Himalayan Digital Library\'s Unicode character based\n\
"Tibetan Machine Uni" OpenType font for writing Tibetan, Dzongkha and\n\
Ladakhi in dbu can script with full support for the Sanskrit\n\
combinations found in chos-skad texts.\n\
tibmachuni-font-ttf home: http://thdl.org/';

testo[a++]='time (the GNU time command for measuring program resource use)\n\
...
testo[a++]='tin (the Tin newsreader)\n\
...
testo[a++]='tix (an extension to the Tk toolkit)\n\
...
with your users in intuitive ways.';

testo[a++]='tk (Tk toolkit for Tcl)\n\
...
testo[a++]='traceroute (IP packet route tracing utility)\n\
...
testo[a++]='transfig (transform .fig files)\n\
...
testo[a++]='tree (a program to display a directory tree)\n\
...
testo[a++]='trn (a threaded newsreader)\n\
...
Be sure to set NNTPSERVER in your /etc/profile or /etc/csh.login. ';

testo[a++]='ttf-indic-fonts (Fonts for Indic scripts)\n\
...
testo[a++]='udev (dynamic device directory system)\n\
...
testo[a++]='unarj (a command-line utility for decompressing .arj archives)\n\
...
testo[a++]='urwid (python console UI module)\n\
...
More info at: http://excess.org/urwid';

testo[a++]='usb_modeswitch (mode switching tool for multiple mode USB devices)\n\
USB-ModeSwitch is (surprise!) a small mode switching tool for\n\
controlling "flip flop" (multiple device) USB gear. These devices\n\
initially appear to be USB storage, typically containing the MS\n\
Windows drivers for whatever the real purpose of the device is, such\n\
as a wireless USB modem. To use these devices with Linux, they need\n\
to be told to switch out of storage mode, which is the purpose of the\n\
USB-ModeSwitch utility.\n\
...
testo[a++]='usbutils (USB utilities)\n\
...
testo[a++]='utempter (utmp updating library and utility)\n\
...
testo[a++]='util-linux (a huge collection of essential utilities)\n\
...
testo[a++]='v4l-utils (libraries and utilities for video4linux)\n\
...
different devices in the same class. Linux V4L2 and DVB API\n\
utilities are also included.\n\
Homepage: http://freshmeat.net/projects/libv4l';

testo[a++]='vbetool (video bios execution tool)\n\
...
testo[a++]='vim-gvim (gvim, the X/GTK2 enabled version of vim)\n\
Gvim is a graphical version of vim. Vim is an almost compatible\n\
version of the UNIX editor vi. Many new features have been added,\n\
such as multi level undo, command line history, filename completion,\n\
block operations, and more.\n\
The main vim package in the AP series is required to use this package.\n\
Vim\'s development is led by Bram Moolenaar.';

testo[a++]='virtuoso-ose (A high-performance object-relational SQL database)\n\
Virtuoso is a scalable cross-platform server that combines\n\
SQL/RDF/XML Data Management with Web Application Server\n\
and Web Services Platform functionality.\n\
Homepage: http://virtuoso.openlinksw.com/wiki/main/';

testo[a++]='vlan (VLAN (802.1q) configuration program)\n\
...
testo[a++]='vorbis-tools (Ogg Vorbis Tools)\n\
...
testo[a++]='vsftpd (Very Secure FTP Daemon)\n\
vsftpd is an FTP server, or daemon. The \'vs\' stands for Very Secure.\n\
Obviously this is not a guarantee, but a reflection that the entire\n\
codebase was written with security in mind, and carefully designed to\n\
be resilient to attack (as well as extremely fast and scalable).\n\
The vsftpd homepage is http://vsftpd.beasts.org/\n\
The Very Secure FTP Daemon was written by Chris Evans.';

testo[a++]='vte (terminal emulator widget)\n\
...
testo[a++]='wavpack (A lossless WAV file compressor)\n\
Wavpack is a hybrid lossless wavefile compressor. Wavfile is\n\
Copyright (c) 1998 - 2006 Conifer Software. All Rights Reserved.\n\
Wavpack is distributed under the BSD Software License.\n\
Visit the wavpack project online: http://www.wavpack.com';

testo[a++]='wget (a non-interactive network retriever)\n\
...
testo[a++]='which (shows the full path to shell commands)\n\
...
testo[a++]='whois (whois directory client)\n\
...
For more information, see: http://www.linux.it/~md/software/';

testo[a++]='wicd (network manager for linux)\n\
...
testo[a++]='windowmaker (a fast and elegant window manager)\n\
...
testo[a++]='wireless-tools (utilities for wireless networking)\n\
...
testo[a++]='workbone (text-based CD player)\n\
WorkBone is a simple interactive tool for for playing audio CDs from\n\
the Linux console.';

testo[a++]='wpa_supplicant (WPA/WPA2/IEEE 802.1X Supplicant)\n\
wpa_supplicant is a WPA Supplicant for Linux with support for WPA and\n\
WPA2 (IEEE 802.11i / RSN). Supplicant is the IEEE 802.1X/WPA\n\
...
testo[a++]='wqy-zenhei-font-ttf (Wen Quan Yi Zen Hei CJK Font)\n\
The WenQuanYi Zen Hei font is a Chinese (or CJK) outline font with\n\
Hei Ti style (a sans-serif style) Hanzi glyphs. This font is\n\
developed for general purpose use of Chinese for formating, printing\n\
and on-screen display. This font is also targeted at platform\n\
independence and the utility for document exchange between various\n\
operating systems.\n\
wqy-zenhei-font-ttf home: http://wqy.sourceforge.net/en/';

testo[a++]='wv2 (MS Word document import library)\n\
...
testo[a++]='x11-skel (Miscellaneous things required for X)\n\
...
testo[a++]='x11-ssh-askpass (an X11-based passphrase dialog)\n\
x11-ssh-askpass is an X11-based passphrase dialog for use\n\
with OpenSSH.';
...
testo[a++]='x3270 (IBM host access tool for X)\n\
...
characters, is a fairly accurate representation of an IBM 3278.';

testo[a++]='xaos (an interactive fractal zoomer)\n\
...
inversion.\n\
Visit XaoS online: http://xaos.sf.net';
...
testo[a++]='xaw3d (3D version of libXaw)\n\
...
testo[a++]='xchat (GTK+-based IRC client)\n\
X-Chat is a GTK+-based IRC (Internet Relay Chat) client.';
...
testo[a++]='xdg-user-dirs (manage XDG user directories)\n\
xdg-user-dirs is a tool used by various XDG compliant desktop\n\
environments to locate user well-known user directories such as\n\
the Desktop folder. It also handles localization/translation of the\n\
filenames.\n\
Homepage: http://freedesktop.org/wiki/Software/xdg-user-dirs';

testo[a++]='xdg-utils (command line tools that assist applications)\n\
...
testo[a++]='xf86-input-acecad\n\
xf86-input-acecad is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-aiptek\n\
xf86-input-aiptek is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-evdev\n\
xf86-input-evdev is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-joystick\n\
xf86-input-joystick is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-keyboard\n\
xf86-input-keyboard is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-mouse\n\
xf86-input-mouse is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-penmount\n\
xf86-input-penmount is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-synaptics\n\
xf86-input-synaptics is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-vmmouse\n\
xf86-input-vmmouse is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-void\n\
xf86-input-void is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-input-wacom\n\
xf86-input-wacom is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-apm\n\
xf86-video-apm is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-ark\n\
xf86-video-ark is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-ast\n\
xf86-video-ast is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-ati\n\
xf86-video-ati is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-chips\n\
xf86-video-chips is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-cirrus\n\
xf86-video-cirrus is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-dummy\n\
xf86-video-dummy is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-glint\n\
xf86-video-glint is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-i128\n\
xf86-video-i128 is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-i740\n\
xf86-video-i740 is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-intel\n\
xf86-video-intel is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-mach64\n\
xf86-video-mach64 is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-mga\n\
xf86-video-mga is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-neomagic\n\
xf86-video-neomagic is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-nouveau\n\
xf86-video-nouveau is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-nv\n\
xf86-video-nv is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-openchrome\n\
xf86-video-openchrome is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-r128\n\
xf86-video-r128 is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-radeonhd (driver for AMD GPG (ATI) R5xx/R6xx chipsets)\n\
xf86-video-radeonhd is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-rendition\n\
xf86-video-rendition is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-s3\n\
xf86-video-s3 is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-s3virge\n\
xf86-video-s3virge is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-savage\n\
xf86-video-savage is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-siliconmotion\n\
xf86-video-siliconmotion is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-sis\n\
xf86-video-sis is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-sisusb\n\
xf86-video-sisusb is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-tdfx\n\
xf86-video-tdfx is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-tga\n\
xf86-video-tga is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-trident\n\
xf86-video-trident is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-tseng\n\
xf86-video-tseng is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-v4l\n\
xf86-video-v4l is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-vesa\n\
xf86-video-vesa is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-vmware\n\
xf86-video-vmware is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-voodoo\n\
xf86-video-voodoo is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86-video-xgi\n\
xf86-video-xgi is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86bigfontproto\n\
...
testo[a++]='xf86dgaproto\n\
xf86dgaproto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86driproto\n\
xf86driproto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86miscproto\n\
xf86miscproto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xf86vidmodeproto\n\
xf86vidmodeproto is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xfce (a fast and lightweight desktop environment for X)\n\
...
testo[a++]='xfce4-notifyd (Xfce Notify Daemon)\n\
Xfce4-notifyd is a simple, visually-appealing notification daemon\n\
for Xfce. Applications can use xfce4-notifyd to pop up a notification\n\
bubble by sending messages through D-Bus.\n\
Homepage: http://spuriousinterrupt.org/projects/xfce4-notifyd';

testo[a++]='xfce4-power-manager (Power Management Daemon for Xfce)\n\
xfce4-power-manager makes power management functions (such as\n\
suspend and hibernate) easy inside the Xfce Desktop Environment\n\
(but it should also work in any other desktop environment).';
...
testo[a++]='xfig (interactive drawing tool)\n\
...
testo[a++]='xfractint (fractal generation program)\n\
...
testo[a++]='xfsdump (backup tools for the XFS filesystem)\n\
...
testo[a++]='xfsprogs (tools to use the XFS filesystem)\n\
...
testo[a++]='xgames (small X games collection)\n\
...
testo[a++]='xine-lib (xine multimedia playback engine libraries)\n\
...
testo[a++]='xine-ui (xine multimedia playback engine user interface)\n\
Xine is a free multimedia player. It plays back CDs, DVDs, and VCDs,\n\
multimedia files, and multimedia streamed over a network.\n\
The xine-ui package contains the graphical user interface for the\n\
xine multimedia player.';

testo[a++]='xineramaproto\n\
...
testo[a++]='xlockmore (screen locker for X)\n\
...
testo[a++]='xmms (X Multimedia System)\n\
XMMS is the X Multimedia System. It is used to play audio and other\n\
kinds of media files. By default XMMS can play MPEG audio, Ogg\n\
...
testo[a++]='xorg-docs\n\
xorg-docs is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xorg-server (The Xorg server, the core of the X Window System)\n\
...
testo[a++]='xorg-server-xephyr (Improved nested X server/client)\n\
Xephyr is a nested X-Client like Xnest, but with some additional\n\
features like XRender support. ';

testo[a++]='xorg-server-xnest (a nested X server)\n\
...
testo[a++]='xorg-server-xvfb (virtual framebuffer X server)\n\
...
testo[a++]='xorg-sgml-doctools\n\
xorg-sgml-doctools is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xpaint (simple X paint program)\n\
XPaint is a versatile bitmap/pixmap editing tool with a feature set\n\
similar to MacPaint.\n\
...
testo[a++]='xsane (Graphical frontend to SANE, Scanner Access Now Easy)\n\
...
testo[a++]='xscope\n\
xscope is part of X11.\n\
For more information about the X.Org Foundation (the providers of the\n\
X.Org implementation of the X Window System), see their website:\n\
http://www.x.org';

testo[a++]='xscreensaver (a screen saver and locker for X)\n\
...
testo[a++]='xterm (terminal emulator for X)\n\
...
testo[a++]='xv (John Bradley\'s interactive image viewer)\n\
...
testo[a++]='xxgdb (graphical program debugger interface)\n\
...
testo[a++]='xz (compression utility based on the LZMA algorithm)\n\
...
and Lasse Collin. For more info: http://tukaani.org/xz/';

testo[a++]='yasm (complete rewrite of the NASM assembler)\n\
...
CodeView 8 formats.\n\
yasm home: http://www.tortall.net/projects/yasm/';

testo[a++]='yptools (NIS servers and clients)\n\
...
testo[a++]='ytalk (multi-user chat program)\n\
...
testo[a++]='zd1211-firmware (Firmware for zd1211 USB wireless)\n\
This distribution contains the firmware files for the ZD1211 chip,\n\
which is used in WLAN USB sticks. The files are generated from C\n\
header files distributed in the original ZyDAS ZD1211 driver. The\n\
GPL-licensed header files used to produce the firmware blobs are also\n\
...
testo[a++]='zlib (compression library)\n\
...
testo[a++]='zoo (archiving and compressing utility)\n\
